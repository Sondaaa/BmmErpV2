        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                   ...
                   <table id="dynamic-table" class="table table-bordered table-hover dynamic-table" style="width: 100%">
                       ...
                       $i=1;
                            foreach ($listes as $l) {
                                $ag = $l;
                                ?>
                                <tr style="cursor: pointer;" id="idde" ondblclick="chargerpersonne2F('<?php echo $ag->getId(); ?>', '<?php echo $ag->getIdrh(); ?>', '<?php echo $ag->getNomcomplet()." ". $ag->getPrenom(); ?>')">
                ...
                <div class="modal-footer" >
                    <button type="button" value="Fermer" class="btn  pull-left" onclick="fermeragentsF()">Fermer</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function chargerpersonne2F(id, mat, nom){ 
        $('#my-modalEmployeF').removeClass('in');
        $('#my-modalEmployeF').css('display', 'none');
        $('#idAgF').val(id);
        $('#agentsF').val(nom);
        $('#idagentsF').val(mat);

    }
    function fermeragentsF(){
        $('#my-modalEmployeF').removeClass('in');
        $('#my-modalEmployeF').css('display', 'none');
    }
