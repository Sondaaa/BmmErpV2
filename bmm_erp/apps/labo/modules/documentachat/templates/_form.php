<div class="sf_admin_form" ng-controller="myCtrlbonstock">
    ...
    <?php  foreach ($configuration->getFormFields($form, $form->isNew() ? 'new' : 'edit') as $fieldset => $fields): ?>
        <?php if (($form->getObject()->isNew() && $idtype == 10) || (!$form->getObject()->isNew() && $form->getObject()->getIdTypedoc())) { ?>  
        ...
        <?php 
        if ($idtype == 11  ) { ?>       
            <?php include_partial('documentachat/form_fieldsetsortie', array('documentachat' => $documentachat, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset,'idtype'=>$idtype, 'iddocparent' => $iddocparent)) ?>
        <?php } ?>
        <?php //die($idtype);
         if ($idtype == 23  ) { ?>       
            <?php include_partial('documentachat/form_fieldsetsortiedef', array('documentachat' => $documentachat, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset,'idtype'=>$idtype, 'iddocparent' => $iddocparent)) ?>
        <?php } ?>
