<?php if ($agents->count() != 0): ?>
    <script type="text/javascript">
        $('#container_poste').highcharts({
            ...
            title: {
                text: 'Statistique des Agents Par poste :'
            },
            tooltip: {
            ...
            series: [{
                    type: 'pie',
                    name: 'Statistique Par Poste',
                    data: [
    <?php for ($i = 0; $i < sizeof($agents); $i++): ?>
                            ['Poste :<?php echo $agents[$i]['poste'] ?>', <?php echo $agents[$i]['nbragents'] ?>],
    <?php endfor; ?>
                    ]
                }]
        });</script>
