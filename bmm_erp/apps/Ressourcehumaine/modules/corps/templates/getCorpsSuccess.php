<?php if ($agents->count() != 0): ?>
    <script type="text/javascript">
        $('#container_corps').highcharts({
            ...
            title: {
                text: 'Statistique des Agents Par Corps  :'
            },
            tooltip: {
            ...
            series: [{
                    type: 'pie',
                    name: 'Statistique Par Corps',
                    data: [
    <?php for ($i = 0; $i < sizeof($agents); $i++): ?>
                            ['Corps :<?php echo $agents[$i]['corps'] ?>', <?php echo $agents[$i]['nbragents'] ?>],
    <?php endfor; ?>
                    ]
                }]
        });</script>
