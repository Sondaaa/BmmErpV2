<?php if (count($pager) == 0): ?>
    <tr>
        <td style=" vertical-align: middle; text-align:center; font-weight: bold; font-size: 18px !important; height: 150px;" colspan="6">Liste des Carrières Vide</td>
    </tr>
<?php endif; ?>
<?php foreach ($pager->getResults() as $contrat): ?>
    <tr>
        <td class="sf_admin_text sf_admin_list_td_id_agents"  style="text-align: left;" >
            <?php echo $contrat->getAgents() ?>
        </td>
        <td class="sf_admin_text sf_admin_list_td_dateemposte"  style="text-align: left;">
            <?php echo $contrat->getPosterh()->getLibelle() ?>
        </td>
        <td class="sf_admin_text sf_admin_list_td_id_fonction"  style="text-align: left;">
            <?php echo $contrat->getUnite()->getLibelle() ?>
        </td>
       
        <td class="sf_admin_text sf_admin_list_td_regroupementagents" style="text-align: left;" >
            <?php echo $contrat->getAgents()->getRegroupementagents()->getLibelle() ;?>
        </td>
            ...
            <div class="btn-toolbar">
                <div class="btn-group" id="btnaction">
                    ...
                    <ul class="dropdown-menu" role="menu">
                        <li>
                            <button type="button" onclick="document.location.href = '<?php echo url_for('contrat/edit?id=' . $contrat->getId() .'&reg='. $contrat->getAgents()->getRegroupementagents()->getId()) ?>'"  class="btn btn-primary width-fixed" >
...
<?php else: ?>
        footer = '<tr>' +
                ...
                '</tr>';
<?php endif; ?>
    $('#listContrat > tfoot').html(footer);
</script>

<style>

    #listContrat tbody td{vertical-align: middle; text-align: center;}
