<div id="sf_admin_container">
    ...
    <div id="sf_admin_content" ng-controller="myCtrldocvisa">  
                ...
                <tbody>
                    <?php foreach ($liste_avis as $lgavis): ?>
                        ...
                        <?php if ($lgavis->getIdLigprotitrub() != null): ?>
                            ...
                            <?php if($lgavis->getLigprotitrub()->getRubrique()->getIdRubrique()):?>
                            <tr>
                                <td style="background: repeat-x #F2F2F2;">Rubrique</td>
                                <td colspan="3"><?php 
                                $rubrique_parent=RubriqueTable::getInstance()->findOneById($lgavis->getLigprotitrub()->getRubrique()->getIdRubrique());
                                                                echo $rubrique_parent; ?></td>
                            </tr>
                            <tr>
                                <td style="background: repeat-x #F2F2F2;">Sous Rubrique</td>
                            ...
                            <tr>
                                ...
                                <td colspan="2"><?php if ($lgavis->getMntdisponible()) echo number_format($lgavis->getMntdisponible(), 3, '.', ' ') . ' TND'; ?></td>
                                <td style="text-align: center;">
                                    <span class="btn btn-success btn-xs" name="btn_edit_table" onclick="showTableAnnexe('<?php echo $lgavis->getLigprotitrub()->getId(); ?>')"><i class="ace-icon fa fa-list bigger-110"></i> Voire Annexe</span>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php endforeach; ?>
        ...
        <?php // if ($ligavissig->count() == 0): ?>
            <fieldset id="zone_visa" style="margin-top: 10px;">
                <legend>Données de Visa</legend>
                <table>
                    <tbody>
                        <tr>
                            <td style="width: 14%; vertical-align: middle;">Liste des Visas B.C.I :</td>
                            <td style="width: 31%">
                                <select id="visaid">
                            ...
                            <td><input type="date" id="datevisa" value="<?php echo date('Y-m-d'); ?>" readonly="true"></td>
                            <td style="text-align: center;">
                                <a ng-click="AjouterVisa(<?php echo $documentachat->getId(); ?>)" class="btn btn-sm btn-outline btn-primary">
                                    <i class="ace-icon fa fa-cc-visa bigger-110"></i> Ajouter Visa
                                </a>
                            </td>
                        </tr>
                        <tr id="zone_motif" style="display: none;">
                            <td style="vertical-align: middle;">Motif d'annulation :</td>
                            <td colspan="6"><input type="text" id="motif" /></td>
                        </tr>
                    </tbody>
                </table>
            </fieldset>
        <?php // endif; ?>
        <fieldset ng-init="ChargerVisa(<?php echo $documentachat->getId() ?>)" style="margin-top: 10px;">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="text-align: center;">Qté. Stock</th>
                        <th>Projet</th>
                        <th>Observation</th>
                    </tr>
                </thead>
                                ...
                                <?php if ($lg->getIdArticlestock()): ?>
                                    ...
                                    <?php if ($article_stocks->count() != 0): ?>
                                        ...
                                        <script  type="text/javascript">
                                            ...
                                            modal: true,
                                                    title: "<div>\n\
                                            <h4 class='smaller'><i class='ace-icon fa fa-archive'></i> Répartition du Stock / Magasin</h4></div>",
                                                    title_html: true,
                                                    buttons: [
                                                    {
                                                    text: "Imprimer",
                                    ...
                                    <?php else: ?>
                                        <a style="padding: 0px 8px;" href="#" id="id-btn-dialog1_<?php echo $lg->getNordre() ?>" class="btn btn-primary btn-sm"><?php echo $total_qte; ?></a>
                                        <div id="dialog-message_<?php echo $lg->getNordre() ?>" class="hide">
                                            <p style="color: #972b2b; font-size: 16px; padding: 20px; margin: 0px;">
                                                Tout les stocks de cet Article sont épuisés.
                                            </p>
                                        </div>
                                        <script  type="text/javascript">
...
<script  type="text/javascript">
            ...
            function showTableAnnexe(id) {
                $.ajax({
                    url: '<?php echo url_for('ligprotitrub/showTableAnnexe') ?>',
                    data: 'id=' + id,
