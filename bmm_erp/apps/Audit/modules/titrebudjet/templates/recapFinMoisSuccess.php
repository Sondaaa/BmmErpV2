<div id="sf_admin_container">
    <h1 id="replacediv"> Engagements Budget 
        <small>
            <i class="ace-icon fa fa-angle-double-right"></i>
            Récapitulatif - Fin du Mois
        </small>
    </h1>
    <div class="panel-body">
        <div class="row">
            <table>
                <tr>
                    <td style="vertical-align: middle; font-weight: bold; width: 50%;">Budget : 
                        <?php $titre_budgets = TitrebudjetTable::getInstance()->getByExercice($_SESSION['exercice_budget']); ?>
                        <select id="titre">
                            <option value="0"></option>
                            <?php foreach ($titre_budgets as $titre_budget): ?>
                                <option value="<?php echo $titre_budget->getId(); ?>"><?php echo $titre_budget; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                    <td style="vertical-align: middle; font-weight: bold; width: 20%;">Jusqu'au Mois :
                        <select id="mois">
                            <option value="0"></option>
                            <option value="1">Janvier ( 31/01 )</option>
                            <option value="2">Février ( <?php echo date('t/m', strtotime(date('Y') . '-02-01')); ?> )</option>
                            <option value="3">Mars ( 31/03 )</option>
                            <option value="4">Avril ( 30/04 )</option>
                            <option value="5">Mai ( 31/05 )</option>
                            <option value="6">Juin ( 30/06 )</option>
                            <option value="7">Juillet ( 31/07 )</option>
                            <option value="8">Août ( 31/01 )</option>
                            <option value="9">Septembre ( 30/09 )</option>
                            <option value="10">Octobre ( 31/10 )</option>
                            <option value="11">Novembre ( 30/11 )</option>
                            <option value="12">Décembre ( 31/12 )</option>
                        </select>
                    </td>
                    <td style="vertical-align: middle; font-weight: bold; width: 15%;">Année :
                        <?php $dates = SituationcumuleeTable::getInstance()->getLimiteDate(); ?>
                        <select id="annee">
                            <option value="0"></option>
                            <?php for ($i = $dates->getMinannee(); $i <= $dates->getMaxannee(); $i++): ?>
                                <option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                            <?php endfor; ?>
                        </select>
                    </td>
                    <td style="vertical-align: middle; text-align: center; width: 15%;">
                        <button onclick="afficher()" class="btn btn-sm btn-primary">
                            <i class="ace-icon fa fa-search bigger-110"></i>
                            <span class="bigger-110 no-text-shadow">Afficher</span>
                        </button>
                    </td>
                </tr> 
            </table>
        </div>
    </div>
</div>

<div class="row" id="etat_recap">

</div>

<script>

    function afficher() {
        if ($('#mois').val() != '0' && $('#titre').val() != '0' && $('#annee').val() != '0') {
            $.ajax({
                url: '<?php echo url_for('titrebudjet/afficherEtatRecap') ?>',
                data: 'mois=' + $('#mois').val() +
                        '&titre=' + $('#titre').val() +
                        '&annee=' + $('#annee').val(),
                success: function (data) {
                    $('#etat_recap').html(data);
                }
            });
        } else {
            bootbox.dialog({
                message: "<span class='bigger-160' style='margin:20px;color:#b31515;'>Veuillez choisir le budget et le mois et/ou l'année !</span>",
                buttons:
                        {
                            "button":
                                    {
                                        "label": "Ok",
                                        "className": "btn-sm"
                                    }
                        }
            });
        }
    }

<?php if ($_SESSION['exercice_budget'] >= date('Y')): ?>
        $(document).ready(function () {
            initialise();
        });
<?php endif; ?>

    function initialise() {
        var mois_courant = '<?php echo intval(date('m')); ?>';
        $('#mois > option').each(function () {
            if (parseInt($(this).val()) > parseInt(mois_courant)) {
                $(this).remove();
            }
        });
    }

</script>