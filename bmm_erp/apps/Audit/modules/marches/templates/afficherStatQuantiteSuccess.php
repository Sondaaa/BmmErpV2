...
<?php for ($i = 0; $i < sizeof($articles); $i++): ?>
                {
                y: <?php echo number_format(($liste[$articles[$i]['id']]['progress'] * 100) / $liste[$articles[$i]['id']]['quetiteant'], 2); ?>,
                        color: colors[0],
                        drilldown: {
    ...
    <?php for ($j = 0; $j < sizeof($liste_detail_prix); $j++): ?>
        ...
        <?php for ($k = 0; $k < sizeof($articles_decompte); $k++): ?>
            <?php if (trim($articles[$i]['libelle']) == trim($articles_decompte[$k]['libelle']) && $liste_detail_prix[$j]['id'] == $articles_decompte[$k]['id_detail']): ?>
                <?php echo number_format(($articles_decompte[$k]['qtemois'] * 100) / $liste[$articles[$i]['id']]['quetiteant'], 2); ?>,
                <?php $trouve = 1; ?>
            <?php endif; ?>
        <?php endfor; ?>
        <?php if ($trouve == 0): ?>
