        ...
        <table id="table_<?php echo $index . '_' . $id; ?>" table_rubrique="<?php echo $index . '_' . $id; ?>" name="tr_table" <?php if ($sous_rubriques->count() == 0): ?>style="display: none;"<?php endif; ?>>
            <thead>
                <tr>
                    ...
                    <?php $typebudget = $sous_rubrique->getTitrebudjet()->getTypebudget(); ?>
            ...
            <tbody id="tbody_rubrique_<?php echo $index . '_' . $id; ?>">
                <?php foreach ($sous_rubriques as $s_rubrique): ?>
                    <?php include_partial('ligprotitrub/ligne_sous_rubrique', array('sous_rubrique' => $s_rubrique, 'index' => $index . '_' . $id)); ?>
                <?php endforeach; ?>
            </tbody>
        </table>
    ...
    <?php if ($sous_rubrique->getTitrebudjet()->getEtatbudget() == 2): ?>
        <?php if (trim($sous_rubrique->getTitrebudjet()->getTypebudget()) != "Prototype"): ?>
            ...
            <td style="text-align: center; width: 13%">
                <span class="btn btn-success btn-xs" name="btn_edit_table" onclick="setTableAnnexe('<?php echo $id; ?>', '<?php echo $index; ?>')" <?php if ($sous_rubriques->count() != 0): ?>style="display: none;"<?php endif; ?>><i class="ace-icon fa fa-list bigger-110 icon-only"></i></span>
            </td>
        <?php endif; ?>
    <?php else: ?>
        <?php if ($sous_rubrique->getTitrebudjet()->getEtatbudget() == 1): ?>
            <td style="text-align: center; width: 13%">
                ...
                <?php if (trim($sous_rubrique->getTitrebudjet()->getTypebudget()) != 'Prototype'): ?>
                    <span class="btn btn-success btn-xs" name="btn_edit_table" onclick="setTableAnnexe('<?php echo $id; ?>', '<?php echo $index; ?>')" <?php if ($sous_rubriques->count() != 0): ?>style="display: none;"<?php endif; ?>><i class="ace-icon fa fa-list bigger-110 icon-only"></i></span>
                <?php endif;?>
                        <span class="btn btn-danger btn-xs" name="btn_remove_rubrique" onclick="removeRubriqueBase('<?php echo $index . '_' . $id; ?>', '<?php echo $index; ?>')"><i class="ace-icon fa fa-remove bigger-110 icon-only"></i></span>
            </td>
        <?php endif; ?>
    <?php endif; ?>
</tr>
