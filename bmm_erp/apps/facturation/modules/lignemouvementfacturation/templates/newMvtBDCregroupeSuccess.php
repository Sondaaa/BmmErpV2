                ...
                <div class="widget-main" style="padding-bottom: 0px; width: 100%; overflow: auto; min-height: 500px" >
                    <table id="idformulaire" class="table table-bordered table-hover" style="width: 100%; margin-bottom: 0px;">
                        <tr>
                            ...
                            <td colspan="5" style="text-align: center; background-color: #dff0d8;">BDC Regroupé </td>
                        </tr>
                        <tr>
                            <td>Date<br>
                            ...
                            <!-- ng-change="AfficheDocAchatFournisseurContrat('#documentachat', '#id_documentachat', '#date_documentachat', '#montant_documentachat_contrat', '#id_fournisseur', '#fournisseur_raison', '#id_ligne')"-->
                            <td>Numéro
                                ...
                                <?php if ($id != '') { ?>
                                    ...
                                    ?>" id="documentachat"  class="disabledbutton">
                                       <?php } else { ?>
                                    <input type="text" value="<?php
                                    if ($id != '') {
                                        echo $numero;
                                    ...
                                    ?>" id="documentachat" ng-model="documentachat.text" 
                                           <?php if ($id == '') { ?>  
                                               onkeydown="chargerlisteBCIduBDCG(event, false)"
                                               ondblclick="chargerlisteBCIduBDCG(event, true)"
                                           <?php } ?>
                                           >
                                       <?php } ?>
                        ...
                        <tr>
                            ...
                            <?php
                            $frss = Doctrine_Core::getTable('fournisseur')->findAll();
                            ?>
                            <td colspan="2">Fournisseur:
                                <select  id="id_fournisseur"   class="chosen-select form-control">
                                    <option value=""></option>
                                    <?php foreach ($frss as $frs): ?>
                                        <option value="<?php echo $frs->getId() ?>">
                                            <?php echo $frs->getRs(); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
...
//                                }
                            ?>" ng-model="fournisseur_raison.text"  id="fournisseur_raison" style="width: 100%" ng-change="AfficheFournisseurMvt('#fournisseur_raison', '#id_fournisseur')">
...
<div id="my-modalListebciBDC" class="modal body" tabindex="1" > 
    <?php
    include_partial('lignemouvementfacturation/listeBciBDCG', array());
    ?>
</div>
<script>
    ...
    function afficher() {
        $.ajax({
            url: '<?php echo url_for('lignemouvementfacturation/afficherMouvementBDCREg') ?>',
...
<script>
    function chargerlisteBCIduBDCG(e, dbclick)
    {
        if (e.keyCode == true)
        {
            var key = e.keyCode;
        ...
        if (key == 112 || dbclick) {
            $('#my-modalListebciBDC').addClass('in');
            $('#my-modalListebciBDC').css('display', 'block');
        }
    }
</script>
