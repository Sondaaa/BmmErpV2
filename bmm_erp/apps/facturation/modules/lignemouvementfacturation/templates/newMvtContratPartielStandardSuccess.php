                ...
                <div class="widget-main" style="padding-bottom: 0px; width: 100%; overflow: auto; min-height: 500px" >
                    <table id="idformulaire" class="table table-bordered table-hover" style="width: 100%; margin-bottom: 0px;">
                        <tr>
                            ...
                            <td colspan="5" style="text-align: center; background-color: #dff0d8;">BCI Contrat </td>
                        </tr>
                        <tr>
                            <td>Date<br>
                            ...
                            <td>Numéro
                                ...
                                <?php if ($id != '') { ?>
                                    ...
                                    ?>" id="documentachat" ng-model="documentachat.text" 
                                           <?php if ($id == '') { ?>  
                                               onkeydown="chargerlisteBCIduContratPartiel(event, false)"
                                               ondblclick="chargerlisteBCIduContratPartiel(event, true)"
                                           <?php } ?>
                                           >
                                       <?php } ?>
...
<div id="my-modalListebciContrat" class="modal body" tabindex="1" > 
    <?php
    include_partial('lignemouvementfacturation/listeBciContrat', array());
    ?>
</div>
<script>
...
<script>
    ...
    function afficher() {
        $.ajax({
            url: '<?php echo url_for('lignemouvementfacturation/afficherMouvementContrat') ?>',
...
<script>
    function chargerlisteBCIduContratPartiel(e, dbclick)
    {
        if (e.keyCode == true)
        {
            var key = e.keyCode;
        ...
        if (key == 112 || dbclick) {
            $('#my-modalListebciContrat').addClass('in');
            $('#my-modalListebciContrat').css('display', 'block');
        }
    }
</script>
<script  type="text/javascript">
    document.title = ("BMM - Facturation : Nouvelle fiche de mouvements");
    $('#lignemouvementfacturation_etatfrs').addClass("chosen-select form-control");
</script>
