            ...
            <?php for ($i = 1; $i <= date('t', strtotime($annee . '-' . $mois . '-01')); $i++): ?>
                ...
                <?php if ($i % 7 == $modulo_after || $i == 1): ?>
                    ...
                    <td id="td_<?php echo $i; ?>">
                        ...
                        $rang_jour_courant = date('N', strtotime($first_date));
                        ?>
                        <?php if ($nom_jour_courant == 'Sat' || $nom_jour_courant == 'Sun'): ?>
                            ...
                            <input class="grille_presence_input" jour="<?php echo $rang_jour_courant; ?>" weekend="<?php echo $nom_jour_courant; ?>" name="semaine_heure" type="text" id="s_<?php echo $i; ?>" semaine="<?php echo $id_semaine ?>" value="" readonly="true" onkeyup="CalculTotal('<?php echo $id_semaine ?>')" onchange="testerabsence('<?php echo $i ?>', '<?php echo $mois ?>', '<?php echo $annee ?>')"/>
                            <input class="grille_presence_input" jour="<?php echo $rang_jour_courant; ?>" weekend="<?php echo $nom_jour_courant; ?>" name="heur_supp" type="text" id="h_<?php echo $i; ?>" semaine="<?php echo $id_semaine ?>" value="" readonly="true" onkeyup="CalculTotalHsup('<?php echo $id_semaine ?>')"/>
                        <?php else: ?>
                            <span type="text" id="codemotif_<?php echo $i; ?>"></span>
                            <input name='jour_motif' type="hidden" id="idmotif_<?php echo $i; ?>">

                            <input class="grille_presence_input" jour="<?php echo $rang_jour_courant; ?>" name="semaine_heure" type="text" type_input="heure_<?php echo $id_semaine ?>" semaine="<?php echo $id_semaine ?>" id="s_<?php echo $i; ?>" value="" onkeyup="CalculTotal('<?php echo $id_semaine ?>')" onchange="testerabsence('<?php echo $i ?>', '<?php echo $mois ?>', '<?php echo $annee ?>')"/><!-- onkeydown="testerabsencezero-->
                            <input class="grille_presence_input" jour="<?php echo $rang_jour_courant; ?>" name="heur_supp" type="text" type_input="supp_<?php echo $id_semaine ?>" semaine="<?php echo $id_semaine ?>" id="h_<?php echo $i; ?>" value="" onkeyup="CalculTotalHsup('<?php echo $id_semaine ?>')"/>
                        <?php endif; ?>
                    </td>
                    <?php $j++; ?>
                    <?php $m++; ?>
                    <?php if ($i % 7 == $after_day || $j == $after_day): ?>
                        <td>
                            ...
                            <input name="total_heure" type="text" id="total_heure_<?php echo $id_semaine ?>" value=""  semaine="<?php echo $id_semaine ?>"/>
                            <input name="total_sup" type="text" id="total_sup_<?php echo $id_semaine ?>" value=""  semaine="<?php echo $id_semaine ?>"/>
                        </td>
                        <?php $id_semaine++; ?>
                    </tr>
                <?php endif; ?>
