<?php if (count($pager) == 0): ?>
    <tr>
        <td style="text-align:center; font-weight: bold; font-size: 16px !important;" colspan="7">Liste des Maquettes Vide</td>
    </tr>
<?php endif; ?>

<?php foreach ($pager->getResults() as $i => $maquette): ?>
    <tr id="ligne_<?php echo $i ?>" onclick="formatLigne(<?php echo $i ?>)" index_ligne="<?php echo $i ?>" ondblclick="showMaquette('<?php echo $maquette->getId() ?>')">
        <td name="ligne_code" style="text-align: center;"><?php echo $maquette->getCode() ?></td>
        <td name="ligne_libelle"><?php echo $maquette->getLibelle() ?></td>
