    ...
    <div class="col-sm-12">
            ...
            <tr>
                <td>
                    ...
                    <select id="compte_max">
                        <option value=''></option>
                        <?php foreach ($comptes as $compte): ?>
                            <option  value="<?php echo $compte->getNumeroCompte() ?>" > <?php echo trim($compte->getNumeroCompte()) . ' - ' . trim($compte->getLibelle()) ?> </option>
                        <?php endforeach; ?>
                    </select>
                </td>
        ...
        <div style="margin-bottom: 15px;" class="mws-panel-body no-padding">
            <div style="height: 360px; overflow: auto;" id="liste_compte">
                <?php include_partial('annulation/list_comptes_plan_dossier', array('comptes' => $comptes)); ?>
            </div>   
        </div>
    </div>
</div>
...
//    }
    ...
    function validerSuppression() {
        ...
        $.ajax({
            url: '<?php echo url_for('annulation/supprimerComptecomptables') ?>',
            data: 'compte_min=' + $('#compte_min').val() + '&compte_max=' + $('#compte_max').val(),
            success: function (data) {
                if (data != null) {
//                    
...
//                    });
    ...
    function afficher() {
        $.ajax({
            url: '<?php echo url_for('annulation/afficherComptescomptables') ?>',
            data: 'compte_min=' + $('#compte_min').val() + '&compte_max=' + $('#compte_max').val(),
            success: function (data) {
