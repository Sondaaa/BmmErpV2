<div id="sf_admin_container">
    <h1 id="replacediv"> Utilitaires
        <small>
            ...
            Annulation Importation des factures comptables Retenue à la source  - <?php echo $_SESSION['exercice']; ?>
        </small>
    </h1>
</div>
<div>
    <div class="col-sm-12">
        ...
        <div style="margin-bottom: 15px;" class="mws-panel-body no-padding">
            <div style="height: 360px; overflow: auto;" id="liste_facture_od">
                <?php include_partial('annulation/liste_od', array('factures' => $factures)); ?>
            </div>   
        </div>
    </div>
</div>
<script  type="text/javascript">
    function supprimer() {
        var message_text = "Voulez-vous supprimer ces factures comptables de retenue à la source ? ";
        bootbox.confirm({
    ...
    function validerSuppression() {
        ...
        $.ajax({
            url: '<?php echo url_for('annulation/supprimerFacturescomptablesOd') ?>',
...
//               
    ...
    function afficher() {
        $.ajax({
            url: '<?php echo url_for('annulation/afficherFacturescomptablesOd')  ?>',
            data: 'facture_min=' + $('#facture_min').val() + '&facture_max=' + $('#facture_max').val(),
            success: function (data) {
                $('#liste_facture_od').html(data);
            }
