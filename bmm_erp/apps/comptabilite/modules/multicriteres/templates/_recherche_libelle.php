<table class="table table-bordered table-hover" style="margin-bottom: 0px;">
    <tr>
        <td style="width: 15%;">Libellé : </td>
        <td style="width: 75%;">
            <input type="text" id="libelle" style="width: 100%;" />
        </td> 
...
<script  type="text/javascript">
    ...
    function rechercher() {
        if ($('#libelle').val() != '') {
            $('#libelle').css('border', '');
            $.ajax({
                url: '<?php echo url_for('multicriteres/rechercheLibelle') ?>',
                data: 'filtre=' + $('#filtre_all').val() + '&libelle=' + $('#libelle').val(),
    ...
    function goPage(page) {
        $.ajax({
            url: '<?php echo url_for('multicriteres/rechercheLibelle'); ?>',
            data: 'filtre=' + $('#filtre_all').val() + '&page=' + page + '&libelle=' + $('#libelle').val(),
            success: function (data) {
                $('#liste_recherche').html(data);
            }
        });
