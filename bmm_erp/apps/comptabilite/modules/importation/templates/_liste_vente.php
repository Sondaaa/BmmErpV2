...
<?php foreach ($pager->getResults() as $facture): ?>
    <tr>
        ...
        <td style="text-align: center;"><?php echo $facture->getClient()->getCodeclt() ?></td>
        <td><?php echo $facture->getClient()->getRs() ?></td>
        <td style="text-align: right;"><?php echo $facture->getTotalhtax() ?></td>
        <td style="text-align: right;"><?php echo $facture->getTfodec() ?></td>
        ...
        <td style="cursor: pointer; text-align: center;">
            <span class="btn-group">
                ...
                <?php if ($facture->getSaisie() == 0 && $facture->getClient()->getIdPlancomptable() != null): ?>
        ...
        <!--                    <a title="Saisir Pièce par Maquette de Saisie" onclick="preparationMaquette('<?php // echo $facture->getId()  ?>')" class="btn btn-xs btn-warning"><i class="ace-icon fa fa-maxcdn"></i></a>-->
                ...
                <?php endif; ?>
                <?php if ($facture->getClient()->getIdPlancomptable() == null): ?>
                    <a title="Affecter Compte comptable" onclick="affecterCompte('<?php echo $facture->getClient()->getId() ?>')" class="btn btn-xs btn-warning"><i class="ace-icon fa fa-creative-commons"></i></a>
                    <input type="hidden" name="affect_compte" value="<?php echo $facture->getClient()->getId(); ?>" id="affect_<?php echo $facture->getId(); ?>"/>
                    <input type="hidden" name="saisir_piece" value="<?php echo $facture->getId(); ?>" id="saisir_<?php echo $facture->getId(); ?>" />
                <?php endif; ?>
            </span>
        </td>
