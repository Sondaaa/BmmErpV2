<div style="font-size: 14px; margin-bottom: 10px">
    <b>Retenue à la source N°:</b> <?php echo $facture->getReference() ?>
</div>
            ...
            <td style="width: 46%">
                ...
                <select id="journal_piece" onchange="setContrePartie()">
                    <?php foreach ($journaux as $journal): ?>
                        <option id="option_<?php echo $journal->getId(); ?>" 
                                contre_id="<?php if ($journal->getIdComptecontrepartie())
                        echo $journal->getIdComptecontrepartie();
                        ?>" 
                                contre="<?php if ($journal->getIdComptecontrepartie())
                                echo $journal->getPlancomptable();
                            ?>" 
                                value="<?php echo $journal->getId(); ?>">
                        <?php echo $journal->getLibelle(); ?>
                        </option>
<?php endforeach; ?>
                </select>
            </td>
...
<table style="width: 100%" class="table table-bordered table-hover" cellspacing="0" cellpadding="0" border="0">
    ...
    <tbody>
        ...
        if ($id_plan != null) {
            $plan_dossier_exer = PlandossiercomptableTable::getInstance()->findOneByIdPlanAndIdExercice($id_plan, $_SESSION['exercice_id']);
            $id_hidden = $plan_dossier_exer->getId();
        }
        ?>
        <tr>
            <td>
                <input type="text" value="<?php echo $plan_dossier_exer; ?>" name="ligne_compte" id="ligne_compte_1" onfocus="chargerCompte('#ligne_compte_1', '#hidden_ligne_compte_1', '#ligne_compte_libelle_1')" onkeyup="chargerCompte('#ligne_compte_1', '#hidden_ligne_compte_1', '#ligne_compte_libelle_1')" onkeydown="moveToNext(event, 'ligne_compte', 1)"/>
                <input type="hidden" value="<?php echo $id_hidden; ?>" name="hidden_ligne_compte" id="hidden_ligne_compte_1" />
                <div name="ligne_compte_libelle" id="ligne_compte_libelle_1" class="mws-form-row" style="text-align: justify; margin-left: 2%;">
                    <?php echo $plan_dossier_exer; ?>
                </div>
            </td>
            <td><input class="align_right" style="width: 100%;" type="text" id="debit_1" value="<?php echo number_format($facture->getTotalttc(), 3, '.', ' ') ?>" readonly="true"/></td>
        ...
        <?php if ($facture->getTotaltva() != 0.000): ?>
            <tr>
                <td>
                    <input type="text" value="" name="ligne_compte" id="ligne_compte_0" onfocus="chargerCompte('#ligne_compte_0', '#hidden_ligne_compte_0', '#ligne_compte_libelle_0')" onkeyup="chargerCompte('#ligne_compte_0', '#hidden_ligne_compte_0', '#ligne_compte_libelle_0')"/>
                    <input type="hidden" value="" name="hidden_ligne_compte" id="hidden_ligne_compte_0" />
                ...
                <td><input class="align_right" style="width: 100%;" id="credit_0" type="text" value="<?php echo number_format($facture->getTotaltva(), 3, '.', ' ') ?>" readonly="true"/></td>
        ...
        <?php endif; ?>
        <?php if ($facture->getTotalht() != 0.000): ?>

            <tr>
                <td>
                    <input type="text" value="" name="ligne_compte" id="ligne_compte_2" onfocus="chargerCompte('#ligne_compte_2', '#hidden_ligne_compte_2', '#ligne_compte_libelle_2')" onkeyup="chargerCompte('#ligne_compte_2', '#hidden_ligne_compte_2', '#ligne_compte_libelle_2')" onkeydown="moveToNext(event, 'ligne_compte', 1)"/>
                ...
                <td><input class="align_right" style="width: 100%;" id="credit_2" type="text" value="<?php echo number_format($facture->getTotalttc(), 3, '.', ' ') ?>" readonly="true"/></td>
