...
<div class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
    ...
    <a class="btn  btn-success pull-right" href="<?php echo url_for('importation/venteExcel') ?>"><i class="ace-icon fa fa-undo"></i> Initialiser l'import</a>
</div>
...
<div id="verif_zone" style="display: none;">
    ...
    <div class="col-xs-12 col-sm-12">
        ...
        <button id="save_button" class="btn  btn-primary pull-right" onclick="saveFacture()"><i class="ace-icon fa fa-save"></i>Enregistrer Tout & Finaliser Import Vente</button>
    </div>
</div>
<script  type="text/javascript">
    ...
    function makeTableParameterFromColumnComptecomptable() {
        ...
        text_table_fournisseur = text_table_fournisseur + '<thead><tr><th>Compte Comptable Tiers  </th></tr></thead>';
        text_table_fournisseur = text_table_fournisseur + '<tbody>';
        $("#res table tbody tr").each(function () {
            if ($(this).find("td:eq(10)").text().trim() != '' ) {
                text_td = $(this).find("td:eq(10)").text().trim();
                ...
                if ($.inArray(text_td, arr_plancomptable) == -1 ) {
                    ...
                    text_table_fournisseur = text_table_fournisseur +
                            ...
                            + $(this).find("td:eq(10)").text().trim() + '</td></tr>';
                }
            }
        });
...
//            libelles = libelles + $(this).find("td:first").text().trim().toUpperCase() + ';;';
            ...
            if (j == next_j) {
                $.ajax({
                    url: '<?php echo url_for('importation/verifPlancomptableClient') ?>',
...
function verifyParametreBase() {
    ...
    function saveFournisseur() {
        ...
        $("#verif_zone_fournisseur table tbody tr").each(function () {
            ...
            if (j == next_j) {
                $.ajax({
                    url: '<?php echo url_for('importation/saveClient') ?>',
...
//            libelles = libelles + $(this).find("td:first").text().trim().toUpperCase() + ';;';
            ...
            if (j == next_j) {
                $.ajax({
                    url: '<?php echo url_for('importation/verifClient') ?>',
//                    data: 'libelles=' + libelles,
                    data: 'codes=' + codes,
                    success: function (data) {
...
//                data: 'libelles=' + libelles,
    ...
    function makeTableParameterFromColumn() {
        ...
        //fournisseur
        var text_table_fournisseur = '<table class="table table-bordered table-hover">';
        text_table_fournisseur = text_table_fournisseur + '<thead><tr><th>Client</th><th>Code</th><th>Compte Comptable</th></tr></thead>';
        text_table_fournisseur = text_table_fournisseur + '<tbody>';
       
   
    $("#res table tbody tr").each(function () { console.log($(this).find("td:eq(3)").html().trim());
            if ($(this).find("td:eq(3)").text().trim() != '') {
                ...
                if ($.inArray(text_td, arr_fournisseur) == -1) {
                    ...
                    text_table_fournisseur = text_table_fournisseur +
                            '<tr id="Four_niss_eur_' + text_td.toUpperCase().replace(/[^a-z0-9]/gi, '') + '"><td>' + text_td + '</td>\n\
                            <td>' + $(this).find("td:eq(2)").text().trim() + '</td>\n\
                            <td>' + $(this).find("td:eq(10)").text().trim() + '</td></tr>';
                }
            }
        });
    ...
    function makeTableFromColumn() {
        ...
        text_table_facture = text_table_facture + '<thead><tr><th style="text-align:center;">Référence</th><th style="text-align:center;">Date</th><th style="text-align:center;">Code Facture</th><th>Client</th>\n\
    <th style="text-align:center;">Total HTAXE</th><th style="text-align:center;">Total Fodec</th><th style="text-align:center;">Total HT</th><th style="text-align:center;">Total TVA</th><th style="text-align:center;">Timbre</th><th style="text-align:center;">Total TTC</th></tr></thead>';
        ...
        var arr_facture = [];
        var text_td_facture = '';
        var total_montant_htax = 0;
        var total_montant_fodec = 0;
            ...
            if (text_td_facture != '') {
                if ($.inArray(text_td_facture, arr_facture) == -1) {
                    ...
                    if ($(this).find('td:eq(4)').html().trim() != '')
                        if (!isNaN(montant_htax))
                        {
                            montant_htax = parseFloat(montant_htax) + parseFloat($(this).find('td:eq(4)').html().trim());
                            total_montant_htax += montant_htax;
                        }
                    if ($(this).find('td:eq(5)').html().trim() != '')
                        if (!isNaN(montant_fodec))
                        {
                            montant_fodec = parseFloat(montant_fodec) + parseFloat($(this).find('td:eq(5)').html().trim());
                            total_montant_fodec += montant_fodec;
                        }
                    if ($(this).find('td:eq(6)').html().trim() != '')
                        if (!isNaN(montant_ht))
                        {
                            montant_ht = parseFloat(montant_ht) + parseFloat($(this).find('td:eq(6)').html().trim());
                            total_montant_ht += montant_ht;
                        }
                    if ($(this).find('td:eq(7)').html().trim() != '')
                        if (!isNaN(montant_tva))
                        {
                            montant_tva = parseFloat(montant_tva) + parseFloat($(this).find('td:eq(7)').html().trim());
                            total_montant_tva += montant_tva;
                        }
                    if ($(this).find('td:eq(8)').html().trim() != '')
                        if (!isNaN(montant_timbre))
                        {
                            montant_timbre = parseFloat(montant_timbre) + parseFloat($(this).find('td:eq(8)').html().trim());
                            total_montant_timbre += montant_timbre;
                        }
                    if ($(this).find('td:eq(9)').html().trim() != '')
                        if (!isNaN(montant_ttc))
                        {
                            montant_ttc = parseFloat(montant_ttc) + parseFloat($(this).find('td:eq(9)').html().trim());
                            total_montant_ttc += montant_ttc;
                        }
                    last_tr_facture = '<tr><td style="text-align:center;">'
                            + $(this).find('td:eq(0)').html().trim() + '</td>\n\
                <td style="text-align:center;">' + $(this).find('td:eq(1)').html().trim() + '</td>\n\
                <td style="text-align:center;">' + $(this).find('td:eq(2)').html().trim() + '</td>\n\
                <td>' + $(this).find('td:eq(3)').html().trim() + '</td>\n\
                <td style="text-align:right;">' + parseFloat(montant_htax).toFixed(3) + '</td>\n\
                <td style="text-align:right;">' + parseFloat(montant_fodec).toFixed(3) + '</td>\n\
                <td style="text-align:right;">' + parseFloat(montant_ht).toFixed(3) + '</td>\n\
                <td style="text-align:right;">' + parseFloat(montant_tva).toFixed(3) + '</td>\n\
                <td style="text-align:right;">' + parseFloat(montant_timbre).toFixed(3) + '</td>\n\
                <td style="text-align:right;">' + parseFloat(montant_ttc).toFixed(3) + '</td></tr>';
        ...
        text_table_facture = text_table_facture + '\
           <tr style="background-color: #DCDCDC"><td></td><td>Total</td><td></td><td></td>\n\
            <td style="text-align:right;">' + parseFloat(total_montant_htax).toLocaleString()  + '</td>\n\
            <td style="text-align:right;">' + parseFloat(total_montant_fodec).toLocaleString()  
            + '</td><td style="text-align:right;">' + parseFloat(total_montant_ht).toLocaleString() 
            + '</td><td style="text-align:right;">' + parseFloat(total_montant_tva).toLocaleString() 
            + '</td><td style="text-align:right;">' + parseFloat(total_montant_timbre).toLocaleString() 
    ...
    function saveFacture() {
        ...
        $("#verif_zone_facture table tbody tr").each(function () {
            ...
            reference = reference + $(this).find("td:eq(0)").text().trim() + ';';
            ...
            if ($(this).find("td:eq(4)").text().trim() != '')
                montant_htax = montant_htax + $(this).find("td:eq(4)").text().trim() + ';';
            else
                montant_htax = montant_htax + '0' + ';';

            if ($(this).find("td:eq(5)").text().trim() != '')
                montant_fodec = montant_fodec + $(this).find("td:eq(5)").text().trim() + ';';
            else
                montant_fodec = montant_fodec + '0' + ';';

            if ($(this).find("td:eq(6)").text().trim() != '')
                montant_ht = montant_ht + $(this).find("td:eq(6)").text().trim() + ';';
            else
                montant_ht = montant_ht + '0' + ';';

            if ($(this).find("td:eq(7)").text().trim() != '')
                montant_tva = montant_tva + $(this).find("td:eq(7)").text().trim() + ';';
            else
                montant_tva = montant_tva + '0' + ';';

            if ($(this).find("td:eq(8)").text().trim() != '')
                montant_timbre = montant_timbre + $(this).find("td:eq(8)").text().trim() + ';';
            else
                montant_timbre = montant_timbre + '0' + ';';

            if ($(this).find("td:eq(9)").text().trim() != '')
                montant_ttc = montant_ttc + $(this).find("td:eq(9)").text().trim() + ';';
            else
            ...
            if (k == next_k) {
                $.ajax({
                    url: '<?php echo url_for('importation/saveFactureVente') ?>',
                    data: 'numero=' + numero + '&reference=' + reference +
                            '&date=' + date +
                            '&fournisseur=' + fournisseur +
                            '&doc=' + doc +
                            '&montant_htax=' + montant_htax +
                            '&montant_fodec=' + montant_fodec +
        ...
        if (k != 0) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $("#save_button").hide();
                    bootbox.dialog({
                        message: "<span class='bigger-160' style='margin:20px;color:#b31531;'></span><br><div class='bigger-110' style='margin:20px;color:#b31531;'>Import Factures Ventes avec succès !!!</div>",
                        buttons:
                                {
                                    "button":
                                            {
