<div id="sf_admin_container">
    <h1 id="replacediv"> Importation 
        <small>
            ...
            Liste des factures comptables retenue à la source Client: <?php echo $_SESSION['exercice']; ?>
        </small>
    </h1>
</div>
...
<div class="row">
    <div class="col-xs-12">
        <div class="table-header" style="margin-bottom: 0px;">
            Liste des Factures Comptables Retenue à la source Client:
            <?php
            ...
            <table id="listFacture" class="mws-datatable-fn mws-table">
                <thead>
                    ...
                    <tr>
                        <th></th>
                        <th></th>
                        <th><input type="text" id="ref" class="align-center" onkeyup="goPageOdClient(1);" /></th>
                        <th><input id="fournisseur" onkeyup="goPageOdClient(1);" type="text" /></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th style="text-align: center;">
                            <a title="Saisir tout les pièces par Maquette de Saisie" href="<?php echo url_for("importation/preparationMaquetteForAll?type=od_client"); ?>" class="btn btn-xs btn-warning"><i class="ace-icon fa fa-maxcdn"></i></a>
                        </th>
                    </tr>
                </thead>
                <tfoot>
                ...
                <tbody>
                    ...
                    <?php include_partial("importation/liste_od_client", array("pager" => $pager, "page" => $page, "reference" => $reference, "fournisseur" => $fournisseur)) ?>
                </tbody>
            </table>
        </div>
    </div>
...
<script  type="text/javascript">
    ...
    function validerCompte(id) {
        if ($('#compte').val() != '') {
            ...
            $.ajax({
                url: '<?php echo url_for('importation/validerCompteOdclient') ?>',
                data: 'compte=' + compte +
                        '&id=' + id +
                        '&reference=' + reference +
    ...
    function affecterCompte(id) {
        $.ajax({
            url: '<?php echo url_for('importation/affecterComptelientOdclient'); ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.confirm({
                    message: data,
    ...
    function goPageOdClient(page) {
        var ref = $('#ref').val();
        var fournisseur = $('#fournisseur').val();
        $.ajax({
            url: '<?php echo url_for('importation/listeOdClient'); ?>',
    ...
    function showFacture(id) {
        $.ajax({
            url: '<?php echo url_for('importation/showOdClient') ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.dialog({
                    message: data,
    ...
    function annulerFacture(id) {
        bootbox.confirm({
            message: "Voulez-vous supprimer cette importation de facture od ?",
            buttons: {
                cancel: {
                    label: "Non",
                    className: "btn-sm",
    ...
    function goAnnulerFacture(id) {
        ...
        var fournisseur = $('#fournisseur').val();
        $.ajax({
            url: '<?php echo url_for('importation/annulerOdClient') ?>',
    ...
    function preparationSaisir(id) {
        $.ajax({
            url: '<?php echo url_for('importation/preparationSaisirFactureOdClient') ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.confirm({
                    message: data,
    ...
    function saisir(id) {
        var id_compte = $("#hidden_ligne_compte_0").val() + ',' + $("#hidden_ligne_compte_1").val();
        var debit = $("#debit_0").val() + ';' + $("#debit_1").val() ;
        var credit = $("#credit_0").val() + ';' + $("#credit_1").val() ;
        var id_contre = $("#hidden_ligne_contre_0").val() + ',' + $("#hidden_ligne_contre_1").val() ;
        $.ajax({
            url: '<?php echo url_for('importation/saisirFactureOdClient') ?>',
            data: 'id=' + id +
                    '&id_compte=' + id_compte +
                    '&debit=' + debit +
                    '&credit=' + credit +
    ...
    function preparationMaquette(id) {
        $.ajax({
            url: '<?php echo url_for('importation/preparationMaquetteOdClient') ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.confirm({
                    message: data,
