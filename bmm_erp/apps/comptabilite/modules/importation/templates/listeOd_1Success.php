<div id="sf_admin_container">
    <h1 id="replacediv"> Importation 
        <small>
            ...
            Liste des factures comptables retenue à la source : <?php echo $_SESSION['exercice']; ?>
        </small>
    </h1>
</div>
...
<div class="row">
    <div class="col-xs-12">
        <div class="table-header" style="margin-bottom: 0px;">
            Liste des Factures Comptables Retenue à la source :
            <?php
            ...
            <table id="listFacture" class="mws-datatable-fn mws-table">
                <thead>
                    <tr style="border-bottom: 1px solid #000000">
                        ...
                        <th style="width: 10%; text-align: center;"> Taux TVA</th>
                    ...
                    <tr>
                        <th></th>
                        <th></th>
                        <th><input type="text" id="ref" class="align-center" onkeyup="goPageOd(1);" /></th>
                        <th><input id="fournisseur" onkeyup="goPageOd(1);" type="text" /></th>
                        <th></th>
                        <th></th>
                        <!--<th></th>-->
<!--                        <th></th>-->
                        <th style="text-align: center;">
                            <a title="Saisir tout les pièces par Maquette de Saisie" href="<?php echo url_for("importation/preparationMaquetteForAll?type=od"); ?>" class="btn btn-xs btn-warning"><i class="ace-icon fa fa-maxcdn"></i></a>
                        </th>
                    </tr>
                </thead>
                <tfoot>
                ...
                <tbody>
                    <?php include_partial("importation/liste_od_1", array("pager" => $pager, "page" => $page, "reference" => $reference, "fournisseur" => $fournisseur)) ?>
                </tbody>
            </table>
        </div>
    </div>
...
<script  type="text/javascript">
    ...
    function goPageOd(page) {
        ...
        var fournisseur = $('#fournisseur').val();
        $.ajax({
            url: '<?php echo url_for('importation/listeOd_1'); ?>',
    ...
    function showFacture(id) {
        $.ajax({
            url: '<?php echo url_for('importation/showOd') ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.dialog({
                    message: data,
    ...
    function goAnnulerFacture(id) {
        ...
        var fournisseur = $('#fournisseur').val();
        $.ajax({
            url: '<?php echo url_for('importation/annulerOd') ?>',
    ...
    function preparationSaisir(id) {
        $.ajax({
            url: '<?php echo url_for('importation/preparationSaisirFactureOd') ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.confirm({
                    message: data,
    ...
    function preparationSaisirBDCReg(id) {
        $.ajax({
            url: '<?php echo url_for('importation/preparationSaisirFactureOdBDCRegroupe') ?>',
            data: 'id=' + id,
            success: function (data) {
                bootbox.confirm({
                    message: data,
    ...
    function saisir(id) {
        ...
        $.ajax({
            url: '<?php echo url_for('importation/saisirFactureOd_1') ?>',
            data: 'id=' + id +
                    '&id_compte=' + id_compte +
                    '&debit=' + debit +
                    '&credit=' + credit +
