...
<div class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
    ...
    <a class="btn  btn-success pull-right" href="<?php echo url_for('importation/achatExcel') ?>"><i class="ace-icon fa fa-undo"></i> Initialiser l'import</a>
</div>
...
<div id="verif_enregistrement" style="display: none;">
    ...
    <div class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
        <button id="verif_parametre" class="btn  btn-primary" onclick="verifyParametreBase()"><i class="ace-icon fa fa-search"></i> Vérifier l'existance des Fournissuers dans la Base des Données</button>
...
<script  type="text/javascript">
    ...
    function makeTableParameterFromColumnComptecomptable() {
        ...
        text_table_fournisseur = text_table_fournisseur + '<thead><tr><th>Compte Comptable Tiers && Compte Comptable de Charge </th></tr></thead>';
...
//            libelles = libelles + $(this).find("td:first").text().trim().toUpperCase() + ';;';
            ...
            if (j == next_j) {
                $.ajax({
                    url: '<?php echo url_for('importation/verifPlancomptable') ?>',
...
//                data: 'libelles=' + libelles,
    ...
    function makeTableFromColumn() {
        ...
        text_table_facture = text_table_facture + '<thead><tr><th style="text-align:center;">Référence</th>\n\
                <th style="text-align:center;">Date</th><th style="text-align:center;">Code Facture</th>\n\
            <th>Fournisseur</th><th style="text-align:center;">Total HT</th>\n\
            <th style="text-align:center;">Total TVA</th><th style="text-align:center;">Timbre</th>\n\
            <th style="text-align:center;">Total TTC</th>\n\
              <th style="text-align:center;">Compte Charge</th><th style="text-align:center;">Libellé </th>\n\</tr></thead>';
...
//                if ($.inArray(text_td_facture, arr_facture) == -1) {
                ...
                last_tr_facture = '<tr><td style="text-align:center;">' + $(this).find('td:eq(0)').html().trim()
                        ...
                        + '</td><td style="text-align:right;">' + parseFloat(montant_ttc).toFixed(3) + '</td>'
                        + '</td><td style="text-align:right;">' + $(this).find('td:eq(9)').html().trim() + '</td>'
                        + '</td><td style="text-align:right;">' + $(this).find('td:eq(10)').html().trim() + '</td>' + ' </tr>';
...
//                }
        ...
        <tr style="background-color: #DCDCDC"><td></td><td>Total</td><td></td>\n\
<td></td><td style="text-align:right;">' + parseFloat(total_montant_ht).toLocaleString() + '</td><td style="text-align:right;">' + parseFloat(total_montant_tva).toLocaleString() + '</td><td style="text-align:right;">' + parseFloat(total_montant_timbre).toLocaleString() + '</td><td style="text-align:right;">' + parseFloat(total_montant_ttc).toLocaleString() + '</td><td></td><td></td></tr></tbody></table>';
        $("#res").fadeOut();
...
//            console.log(date + 'date');
            ...
            libelle = libelle + $(this).find("td:eq(9)").text().trim() + ';';
            comptes_charge = comptes_charge + $(this).find("td:eq(8)").text().trim() + ';';
//        console.log(comptes_charge) ;
            k++;
            if (k == next_k) {
                var data = {
                    numero: numero,
                ...
                $.ajax({
                    ...
                    url: '<?php echo url_for('importation/saveFactureAchat') ?>',
...
//                    {
                        ...
                        $("#save_button").hide();
                        bootbox.dialog({
                            message: "<span class='bigger-160' style='margin:20px;color:#b31531;'></span><br><div class='bigger-110' style='margin:20px;color:#b31531;'>Import Factures Achats avec succès !!!</div>",
                            buttons:
                                    {
                                        "button":
                                                {
