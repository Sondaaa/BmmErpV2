...
<?php foreach ($pager->getResults() as $facture): ?>
    <tr>
        ...
        <td style="text-align: right;"><?php echo number_format($facture->getTimbre() , 3, '.', ' ') ?></td>
        ...
        <td style="text-align: center;">
           ...
           <!--<a onclick="openPopupAnnuler(<?php // echo $facture->getId() ?>)" class="btn btn-danger btn-xs"><i class="ace-icon fa fa-trash"></i></a>-->
       
    <!--<a title="Supprimer" onclick="openPopupAnnuler(<?php // echo $facture->getId() ?>)" class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash"></i></a>-->
        </td>
    </tr>
<?php endforeach; ?>
...
<?php if ($pager->haveToPaginate()): ?>
    ...
    <?php else: ?>
            '<li class ="paginate_button previous" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_previous"> <a onclick="goPageTOD(\'1\')"> <i class="ace-icon fa fa-angle-double-left"></i> Première </a></li>' +
                    '<li class ="paginate_button previous" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_previous"> <a onclick="goPageTOD(\'<?php echo $pager->getPreviousPage() ?>\')"> <i class="ace-icon fa fa-angle-left"></i> Précédente </a></li>' +
    <?php endif; ?>
    <?php foreach ($pager->getLinks() as $page): ?>
        <?php if ($page == $pager->getPage()): ?>
                '<li class ="paginate_button active" aria-controls ="dynamic-table" tabindex ="0"> <a href = "#"> <?php echo $page ?> </a></li>' +
        <?php else: ?>
                '<li class ="paginate_button" aria-controls ="dynamic-table" tabindex ="0"> <a onclick="goPageTOD(\'<?php echo $page ?>\')"> <?php echo $page ?> </a></li>' +
        <?php endif; ?>
    <?php endforeach; ?>
    ...
    <?php else: ?>
            '<li class ="paginate_button next" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_next"> <a onclick="goPageTOD(\'<?php echo $pager->getNextPage() ?>\')"> Suivante <i class="ace-icon fa fa-angle-right"></i> </a></li>' +
                    '<li class ="paginate_button next" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_next"> <a onclick="goPageTOD(\'<?php echo $pager->getLastPage() ?>\')"> Dernière <i class="ace-icon fa fa-angle-double-right"></i> </a></li>' +
    <?php endif; ?>
        '</ul>' +
                '</div>' +
                '</div>' +
