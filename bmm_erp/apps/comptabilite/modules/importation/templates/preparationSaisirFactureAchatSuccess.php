<div style="font-size: 14px; margin-bottom: 10px">
    <b>Facture Achat N°:</b> 
    <?php
    echo $facture->getReference();
//    if ($dossier->getIdCompteachat()) {
//                $id_plan = $dossier->getIdCompteachat();
//                $plan_dossier = PlandossiercomptableTable::getInstance()->findOneByIdDossierAndIdExerciceAndIdPlan($_SESSION['dossier_id'], $_SESSION['exercice_id'], $id_plan);
//            echo ($plan_dossier->getId().'id');}
    ?>
</div>
...
<table style="width: 100%;" class="table table-bordered table-hover" cellspacing="0" cellpadding="0" border="0">
    ...
    <tbody>
        <tr>
            ...
            <td <?php if (!$dossier->getIdCompteachat()): ?>name="contre_partie" td_contre_id="0"<?php endif; ?>>
                ...
                <div name="ligne_compte_libelle" id="ligne_compte_libelle_0" class="mws-form-row" style="text-align: justify; margin-left: 2%;">
                    <?php if ($dossier->getIdCompteachat()) echo $dossier->getCompteAchat(); ?>
                </div>
            </td>
            <?php if ($_SESSION['dossier_id'] != 1): ?>
                <td><input class="align_right" style="width: 100%;" type="text" id="debit_0" value="<?php echo number_format($facture->getTotalht() + $facture->getTimbre(), 3, '.', ' ') ?>" readonly="true"/></td>
            <?php else: ?>
                <td><input class="align_right" style="width: 100%;" type="text" id="debit_0" value="<?php echo number_format($facture->getTotalttc(), 3, '.', ' ') ?>" readonly="true"/></td>
            <?php endif; ?>
        ...
        <tr>
            <td>
                <?php
                if ($facture->getFournisseur()->getIdPlancomptable()) {
                    $id_plan = $facture->getFournisseur()->getIdPlancomptable();
                    $plan_dossier = PlandossiercomptableTable::getInstance()->findOneByIdDossierAndIdExerciceAndIdPlan($_SESSION['dossier_id'], $_SESSION['exercice_id'], $id_plan);
                }
                ?>
                <input type="text" value="<?php echo $facture->getFournisseur()->getPlancomptable(); ?>" name="ligne_compte" id="ligne_compte_2" onfocus="chargerCompte('#ligne_compte_2', '#hidden_ligne_compte_2', '#ligne_compte_libelle_2')" onkeyup="chargerCompte('#ligne_compte_2', '#hidden_ligne_compte_2', '#ligne_compte_libelle_2')" onkeydown="moveToNext(event, 'ligne_compte', 1)"/>

                <input type="hidden" value="<?php echo $plan_dossier->getId(); ?>" name="hidden_ligne_compte" id="hidden_ligne_compte_2" />
