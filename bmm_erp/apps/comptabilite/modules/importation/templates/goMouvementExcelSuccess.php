...
<div class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
    ...
    <a class="btn  btn-success pull-right" href="<?php echo url_for('importation/mouvementExcel') ?>"><i class="ace-icon fa fa-undo"></i> Initialiser l'import</a>
</div>
...
<div id="verif_zone" style="display: none;">
    <legend>Tableau Mouvement : <span id="count_facture"></span> <span style="font-size: 14px;">Règlement</span></legend>
    <div class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
    ...
    <div class="col-xs-12 col-sm-12">
        ...
        <button id="save_button" class="btn  btn-primary pull-right" onclick="saveFacture()"><i class="ace-icon fa fa-save"></i>Enregistrer Tout & Finaliser Import Mouvement</button>
    </div>
</div>
    ...
    function makeTableParameterFromColumnComptecomptable() {
        ...
        $("#res table tbody tr").each(function () {
            if ($(this).find("td:eq(5)").text().trim() != '') {
                text_td = $(this).find("td:eq(5)").text().trim();
                ...
                if ($.inArray(text_td, arr_plancomptable) == -1) {
                    ...
                    text_table_fournisseur = text_table_fournisseur +
                            ...
                            + $(this).find("td:eq(5)").text().trim() + '</td></tr>';
                }
            }
        });
...
//            libelles = libelles + $(this).find("td:first").text().trim().toUpperCase() + ';;';
    ...
    function checkDateAndFloat() {
        $('#res table tbody tr').each(function () {
            ...
            parseFloat($(this).find('td:eq(3)').html().trim()).toFixed(3);
...
//            parseFloat($(this).find('td:eq(6)').html().trim()).toFixed(3);
    ...
    <th style="text-align:center;">Numèro pièce</th><th style="text-align:center;">Montant </th>\n\
...
//                if ($.inArray(text_td_facture, arr_facture) == -1) {
                ...
                if ($(this).find('td:eq(3)').html().trim() != '')
                    if (!isNaN(montant_ht))
                    {
                        montant_ht = parseFloat(montant_ht) + parseFloat($(this).find('td:eq(3)').html().trim());
...
//                }
        ...
        text_table_facture = text_table_facture + '\
           ...
           <td style="text-align:right;">' + parseFloat(total_montant_ht).toLocaleString()
    ...
    function saveFacture() {
        ...
        $("#verif_zone_facture table tbody tr").each(function () {
            ...
            if ($(this).find("td:eq(3)").text().trim() != '')
                montant_ht = montant_ht + $(this).find("td:eq(3)").text().trim() + ';';
            else
                montant_ht = montant_ht + '0' + ';';

            type = type + $(this).find("td:eq(4)").text().trim() + ';';
            numero_compte = numero_compte + $(this).find("td:eq(5)").text().trim() + ';';
            k++;
//            console.log(numero + ' ' + type);
            if (k == next_k) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo url_for('importation/saveMouvementcomptable') ?>',
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.msg == 'OK')
                    {
                        ...
                        $("#save_button").hide();
                        bootbox.dialog({
                            message: "<span class='bigger-160' style='margin:20px;color:#b31531;'></span><br><div class='bigger-110' style='margin:20px;color:#b31531;'>Import Mouvement Comptable avec succès !!!</div>",
                            buttons:
                                    {
                                        "button":
                                                {
