...
<div class="row">
    <div class="col-xs-6">
        <div class="widget-box">
            <div class="widget-header widget-header-flat">
                <h4 class="widget-title smaller">Ajouter Dossier Comptable</h4>
            </div>

            <div class="widget-body">
                <div class="widget-main">
                    <form method="post" action="<?php echo url_for('@ajouterdossier') ?>" enctype="multipart/form-data">
                  
                        <fieldset>
                            <table>
                                <tr>
                                    <td><label>Dossier Comptable* :</label></td>
                                    <td> <input id="libelle" name="libelle" type="text" value="" class="form-control" placeholder="Libellé " />
                                    </td>

                                </tr>
                                <tr>
                                    <td><label>Choisir  Dossier </label></td>
                                    <td>  <input name="lib_fichier_dossier" id="lib_fichier_dossier" type="file" >
    ...
    <div class="col-xs-6" id="zone_edit" style="display: none;">
        <div class="widget-box widget-color-grey">
            <div class="widget-header widget-header-flat">
                <h4 class="widget-title smaller">Modifier Dossier Comptable</h4>
            </div>
            ...
            <table id="list_forme" class="mws-datatable-fn mws-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 20%;">Chemin Dossier</th>
                ...
                <tbody>
                    <?php include_partial('referentielcomptable/liste_dossierutile', array('pager' => $pager)) ?> 
                </tbody>
            </table>
        </div>
    </div>
...
<script  type="text/javascript">
    ...
    function goPage(page) {
        $.ajax({
            url: '<?php echo url_for('@dossierutile') ?>',
...
//    function ajouter() {
//        var filename = $('#lib_fichier_dossier').val().replace(/.*(\/|\\)/, '');
//        if ($('#libelle').val() != '' || $('#lib_fichier_dossier').val() != '') {
//            $('#libelle').css('border', '');
//            $.ajax({
//                url: '<?php echo url_for('@ajouterdossier') ?>',
//                data: 'new_libelle=' + $('#libelle').val() + '&url=' + filename,
//                success: function (data) {
//                    $('#libelle').val('');
//                    $('#lib_fichier_dossier').val('');
//                    $('#list_forme > tbody').html(data);
//                }
//            });
//        }
...
//            $('#lib_fichier_dossier').css('border-color', '#f2a696');
//        }
//    }
    ...
    function validerSuppression(id) {
        $.ajax({
            url: '<?php echo url_for('@deleteDossier') ?>',
            data: 'id=' + id,
            success: function (data) {
                $('#list_forme > tbody').html(data);
            }
    ...
    function editForme(id) {
        $.ajax({
            url: '<?php echo url_for('@editDossier') ?>',
            data: 'id=' + id,
            success: function (data) {
...
//            $.ajax({
//                url: '<?php echo url_for('@updateDossier') ?>',
