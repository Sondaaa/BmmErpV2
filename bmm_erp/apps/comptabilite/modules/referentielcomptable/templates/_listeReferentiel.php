<?php if ($pager->count() == 0): ?>
    <tr>
        <td style="text-align:center; font-weight: bold; font-size: 16px !important;" colspan="2">Liste des Rèfèrenteils Comptables  vide</td>
    </tr>
<?php endif; ?>
<?php foreach ($pager->getResults() as $refrentiel): ?>
    <tr>
        <td><?php echo $refrentiel->getLibelle(); ?></td>
        <td style="text-align: center;">
            <button type="button" class="btn btn-white btn-info btn-sm" onclick="editForme('<?php echo $refrentiel->getId() ?>')" style="text-align: center;">
                Modifier
                <i class="ace-icon fa fa-edit icon-on-right bigger-110"></i>
            </button>
         
                <button type="button" class="btn btn-white btn-danger btn-sm" onclick="deleteForme('<?php echo $refrentiel->getId() ?>')" style="text-align: center;">
                    Supprimer
