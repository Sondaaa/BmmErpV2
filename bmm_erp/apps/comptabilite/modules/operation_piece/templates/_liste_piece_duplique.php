<?php if (count($pager) == 0): ?>
    <tr>
        <td style="text-align:center; font-weight: bold; font-size: 16px !important;" colspan="9">Liste des Pièces Dupliquées Vide</td>
    </tr>
<?php endif; ?>
...
<?php foreach ($pager->getResults() as $i => $piece): ?>
    <tr id="ligne_<?php echo $i ?>" onclick="formatLigne(<?php echo $i ?>)" index_ligne="<?php echo $i ?>" ondblclick="showPiece('<?php echo $piece->getId() ?>')">
        ...
        <td name="ligne_numero" style="text-align: center;">
             <a style="cursor: pointer;" target="_blank" title="Modifier Pièce" href="<?php echo url_for('saisie_pieces/showEdit?id=' . $piece->getId()) ?>">
                                                <?php echo $piece->getNumero() ?>
                                            </a>
                                                <?php // echo $piece->getNumero() ?></td>
        ...
        <td style="text-align: center;">
            ...
            <a style="cursor: pointer" href="<?php echo url_for('operation_piece/showSource?id=' . $piece->getIdPiecesource()); ?>" target="_blank"><?php echo $piece_source->getNumero() ?> </a>
        </td>
        <td style="cursor: pointer; text-align: center;">
            <span class="btn-group">
                <a style="cursor: pointer" title="Afficher" onclick="showPiece('<?php echo $piece->getId() ?>')" class="btn btn-xs btn-primary"><i class="ace-icon fa fa-eye"></i> Afficher</a>
            </span>
        </td>
    </tr>
<?php endforeach; ?>
