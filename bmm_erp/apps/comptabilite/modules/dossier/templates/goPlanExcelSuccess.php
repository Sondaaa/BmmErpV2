...
<div id="sf_admin_container">
    <h1 id="replacediv"> Plan Comptable  
...
<div class="col-xs-12 col-sm-12" style="margin-bottom: 10px;">
    ...
    <a class="btn  btn-success pull-right" href="<?php echo url_for('dossier/importerPlan') ?>"><i class="ace-icon fa fa-undo"></i> Initialiser l'import</a>
</div>
...
<div id="verif_zone" style="display: none;">
    <div class="col-xs-12 col-sm-12" style="margin-top: 15px;">
        ...
        <button id="save_button" class="btn  btn-primary pull-right" onclick="savePlan()"><i class="ace-icon fa fa-save"></i>Enregistrer Tout & Finaliser Import Plan Comptable</button>
    </div>
</div>
    ...
    function savePlan() {
        ...
        $("#res table tbody tr").each(function () {
            ...
            if (k == next_k) {
                ...
                $.ajax({
                    url: '<?php echo url_for('dossier/savePlanComptable') ?>',
                    data: 'code=' + code +
                            '&libelle=' + libelle +
                            '&classe=' + classe,
  ...
  function savePlanDossier() {
        ...
        $("#res table tbody tr").each(function () {
            ...
            if (isNaN(myString))
                ...
                solde = solde + myString + ';';

            code_parent = code_parent + $(this).find("td:eq(4)").text().trim() + ';';

            k++;
            if (k == next_k) {
                $.ajax({
                    url: '<?php echo url_for('dossier/savePlanDossierComptable') ?>',
                    data: 'solde=' + solde +
                            '&code=' + code +
                            '&libelle=' + libelle +
            ...
            $.ajax({
                ...
                success: function (data) {
                    $('#loading_save_icon').fadeOut();
                     ...
                     bootbox.dialog({
                message: "<span class='bigger-160' style='margin:20px;color:#b31531;'></span><br><div class='bigger-110' style='margin:20px;color:#b31531;'>Plan Comptable est importé avec succée !!!</div>",
                buttons:
                        {
                            "button":
                                    {
