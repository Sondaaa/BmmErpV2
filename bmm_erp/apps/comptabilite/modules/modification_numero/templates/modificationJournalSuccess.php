<div id="sf_admin_container">
    <h1 id="replacediv"> Utilitaires
        <small>
            ...
            Modification journal comptable - Exercice <?php echo $_SESSION['exercice']; ?>
        </small>
    </h1>
</div>
                ...
                <table id="listPiece" class="mws-datatable-fn mws-table">
                    <thead>
                        <tr id="list_tri">
                            ...
                            <th style="width: 20%; text-align: center;">Nouveau Journal</th>
...
<script  type="text/javascript">
    ...
    function getPieces() {
        ...
        type = 'ligne_' + type;
        $.ajax({
            url: '<?php echo url_for('modification_numero/getPiecesJournalForJournal'); ?>',
    ...
    function showJournal(id) {
        $.ajax({
            url: '<?php echo url_for('modification_numero/showFormDateForJournal') ?>',
    ...
    function setNewValues(id) {
        ...
        var text_new_date = '<i class="ace-icon fa fa-pencil-square-o bigger-110"></i> ' + $('#new_journal option:selected').text();
        $('#text_new_journal_' + id).html(text_new_date);
        $('#journal_' + id).val($('#new_journal').val());
...
<script  type="text/javascript">
    document.title = ("BMM - G. Compta. : U. Modification journal comptable");
</script>