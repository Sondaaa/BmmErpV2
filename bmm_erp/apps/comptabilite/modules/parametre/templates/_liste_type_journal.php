<?php if ($pager->count() == 0): ?>
    <tr>
        <td style="text-align:center; font-weight: bold; font-size: 16px !important;" colspan="2">Liste des types des journaux vide</td>
    </tr>
<?php endif; ?>
<?php foreach ($pager->getResults() as $type_journal): ?>
    <tr>
        <td>
            <?php echo $type_journal->getLibelle() ?> 
            <?php if ($type_journal->getId() == 1 || $type_journal->getId() == 2 || $type_journal->getId() == 3  || $type_journal->getId() == 5): ?>
                <a class="grey" data-tootip="show_tooltip" href="#" title="On ne peut, ni modifier, ni supprimer le type de journal comptable : <?php echo $type_journal->getLibelle() ?>">
        ...
        <td style="text-align: center;">
            <?php if ($type_journal->getId() != 1 && $type_journal->getId() != 2 && $type_journal->getId() != 3 && $type_journal->getId() != 5 ): ?>
                <button type="button" class="btn btn-white btn-info btn-sm" onclick="editForme('<?php echo $type_journal->getId() ?>')" style="text-align: center;">
                    Modifier
                    <i class="ace-icon fa fa-edit icon-on-right bigger-110"></i>
                </button>
                <?php if ($type_journal->getJournalcomptable()->count() == 0): ?>
                    <button type="button" class="btn btn-white btn-danger btn-sm" onclick="deleteForme('<?php echo $type_journal->getId() ?>')" style="text-align: center;">
                        Supprimer
