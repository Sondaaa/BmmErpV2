...
<div class="row">
    <div class="col-xs-6">
        <div class="widget-box">
            <div class="widget-header widget-header-flat">
                <h4 class="widget-title smaller">Ajouter forme juridique</h4>
            </div>
                ...
                <div class="widget-main">
                    <form>
                        <fieldset>
                            ...
                            <input id="libelle" placeholder="Libellé forme juridique" type="text" value="" class="form-control" />
                        </fieldset>
    ...
    <div class="col-xs-6" id="zone_edit" style="display: none;">
        <div class="widget-box widget-color-grey">
            <div class="widget-header widget-header-flat">
                <h4 class="widget-title smaller">Modifier forme juridique</h4>
            </div>
    ...
    <div class="col-xs-12">
        ...
        <div>
            <table id="list_forme" class="mws-datatable-fn mws-table">
                <thead>
                    <tr>
                        <th style="width: 80%;">Forme juridique</th>
                ...
                <tbody>
                    <?php include_partial('parametre/liste_forme_juridique', array('pager' => $pager)) ?> 
                </tbody>
            </table>
        </div>
    </div>
...
<script  type="text/javascript">
    ...
    function goPage(page) {
        $.ajax({
            url: '<?php echo url_for('@formeJuridique') ?>',
    ...
    function ajouter() {
        if ($('#libelle').val() != '') {
            $('#libelle').css('border', '');
            $.ajax({
                url: '<?php echo url_for('@ajouterForme') ?>',
    ...
    function deleteForme(id) {
        bootbox.confirm({
            message: "Voulez-vous supprimer cette forme juridique ?",
            buttons: {
                cancel: {
                    label: "Non",
                    className: "btn-sm",
    ...
    function validerSuppression(id) {
        $.ajax({
            url: '<?php echo url_for('@deleteForme') ?>',
            data: 'id=' + id,
            success: function (data) {
                $('#list_forme > tbody').html(data);
            }
    ...
    function editForme(id) {
        $.ajax({
            url: '<?php echo url_for('@editForme') ?>',
            data: 'id=' + id,
            success: function (data) {
    ...
    function modifier(id) {
        if ($('#libelle_edit').val() != '') {
            ...
            $.ajax({
                url: '<?php echo url_for('@updateForme') ?>',
...
<script  type="text/javascript">
    document.title = ("BMM - G. Compta. : Forme juridique");
</script>