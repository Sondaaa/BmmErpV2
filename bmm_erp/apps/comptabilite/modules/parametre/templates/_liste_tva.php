<?php if ($pager->count() == 0): ?>
    <tr>
        <td style="text-align:center; font-weight: bold; font-size: 16px !important;" colspan="3">Liste des T.V.A vide</td>
    </tr>
<?php endif; ?>
<?php foreach ($pager->getResults() as $tva): ?>
    <tr>
        <td style="text-align: center;"><?php echo $tva->getLibelle(); ?></td>
        <td style="text-align: center;"><?php echo $tva->getValeurtva(); ?></td>
        <td style="text-align: center;">
            <?php if ($tva->getArticle()->count() >= 0 ): ?>
                <button type="button" class="btn btn-white btn-info btn-sm" onclick="editForme('<?php echo $tva->getId() ?>')" style="text-align: center;">
                    Modifier
                    <i class="ace-icon fa fa-edit icon-on-right bigger-110"></i>
                </button>
            
                <button type="button" class="btn btn-white btn-danger btn-sm" onclick="deleteForme('<?php echo $tva->getId() ?>')" style="text-align: center;">
                    Supprimer
