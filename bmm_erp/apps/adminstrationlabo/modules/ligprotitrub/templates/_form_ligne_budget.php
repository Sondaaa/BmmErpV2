...
$categorietitre=CategorietitreTable::getInstance()->findOneById($formdetail->getObject()->getIdCat());
        ...
        <table class="disabledbutton">
            <tr>
                <td>
                    ...
                    <input type="text" id="mnt_tranche" value="<?php echo $tranche->getMntvaleur() ?>">
                </td>

                <td>
                    <label>Reste du Montant du tranche</label>
    ...
    <div class="col-lg-7" style="float: left; margin-top: 15px; margin-bottom: 15px;">
        <?php if (!$formdetail->getObject()->isNew() && trim($formdetail->getObject()->getTypebudget()) != "Prototype") { ?>
            ...
            <?php
                    ...
                    $par = $tranche;
                    $class = "";
                     if(count($alimentations)==0 || $categorietitre->getIsBudgetetat())
                     $class = "disabledbutton";
                    
                    $tranche_ajout_id = $tranche->getId();
                    ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr class="<?php echo $class ?>" ng-model="tr_<?php echo $tranche->getId() ?>">
                        ...
                        <td>
                            <input type="date" id="date_tran_<?php echo $par->getId() ?>" value="<?php if ($tranche) echo $tranche->getDatetranche() ?>">
                        </td>
                        <td>
                            <input ng-model="mnt_tr_<?php echo $par->getId() ?>" type="text" id="mnt_tr_<?php echo $par->getId() ?>" ng-change="CalculPourcentage(<?php echo $par->getId() ?>)" <?php /*if(count($alimentations)>0):*/ ?> value="<?php /*if ($tranche)*/ echo $tranche->getMntvaleur() ?>" ng-value="<?php /*if ($tranche)*/ echo $tranche->getMntvaleur() ?>" <?php /*endif*/ ?>>
                        </td>
                        <td>
                            <input type="text" ng-model="mnt_pour_ence_<?php echo $par->getId() ?>" ng-change="CalculMntParPourcentage(<?php echo $par->getId() ?>)" id="mnt_pour_ence_<?php echo $par->getId() ?>" <?php /*if(count($alimentations)>0):*/ ?> value="<?php /*if ($tranche)*/ echo $tranche->getMntpourcentage() ?>" ng-value="<?php /*if ($tranche)*/ echo $tranche->getMntpourcentage() ?>" <?php /*endif*/ ?>>
                        </td>
                        <?php if($class==""): ?>
                        <td>
                ...
                <div class="modal-dialog" style="width: 1000px;">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-lg-3">
                                    ...
                                    <input type="text" id="montant_encaisse_rubrique" value="<?php if ($tranche) echo $tranche->getMntvaleur() ?>">
                                </div>
                                <div class="col-lg-2">
                                    Taux Encaissement :
                                    <input type="text" id="taux_encaisse_rubrique" value="<?php if ($tranche) echo '100' ?>">
                                </div>
                                <div class="col-lg-3">
                                    Montant Reste :
                            ...
                            <table>
                                ...
                                <tbody>
                                    <?php foreach ($rubriques as $rubrique) : ?>
                                        <?php include_partial('ligprotitrub/ligne_rubrique_alimentation', array('rubrique' => $rubrique,'tranche'=>$tranche)); ?>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        </div>
                        <div class="modal-footer">
                            ...
                            <button class="btn btn-sm btn-primary pull-right" data-dismiss="modal" ng-click="AlimenterRubriques('<?php echo $formdetail->getObject()->getId() ?>','<?php echo $tranche->getId() ?>')">
                ...
                <table id="list_alimentation">
                    <thead>
                        ...
                        <tr>
                            ...
                            <?php if(!$categorietitre->getIsBudgetetat()):?>
                            <th style="width: 6%;"></th>
                            <?php endif;?>
    ...
    <table id="budget_rubrique" name="tr_table" table_rubrique="">
        <thead>
            <tr>
                ...
                <th style="width: 45%">RUBRIQUES</th>
                <?php if (!$formdetail->getObject()->isNew() && trim($formdetail->getObject()->getTypebudget()) != "Prototype") { ?>
    ...
    <div class="row" style="padding-right: 2%; margin-top: 20px; text-align: right;">
        <?php if (!$tranche->getEtattranche()) : ?>
            <?php if ($typebudget != 'Prototype') : ?>
                <input type="button" value="Enregistrer tranche" onclick="ValiderSousDetail('<?php echo $formdetail->getObject()->getId() ?>', '1','<?php echo $tranche->getId() ?>')">
                <?php if ($formdetail->getObject()->getEtatbudget() == 1) : ?>
                    <input type="button" value="Enregistrer & Clôturer tranche" onclick="ValiderSousDetail('<?php echo $formdetail->getObject()->getId() ?>', '3','<?php echo $tranche->getId() ?>')">
                <?php endif; ?>
            <?php else : ?>

                <input type="button" value="Valider tranche" onclick="ValiderSousDetail('<?php echo $formdetail->getObject()->getId() ?>', '3','<?php echo $tranche->getId() ?>')">
                <input type="button" value="Valider & Clôturer tranche" onclick="ValiderSousDetail('<?php echo $formdetail->getObject()->getId() ?>', '2','<?php echo $tranche->getId() ?>')">
            <?php endif; ?>
        <?php endif; ?>
...
<script type="text/javascript">
                    ...
                    if (verif_montant) {
                        ...
                        $.ajax({
                            ...
                            data: 'id=' + id +
                                ...
                                '&id_tranche=' + '<?php echo $tranche->getId() ?>' +
    ...
    function removeRubriqueBase(id, index) {
        ...
        $.ajax({
            ...
            data: 'id=' + id_rubrique + '&index=' + index+'&trancheid=<?php echo $tranche->getId() ?>',
            success: function(data) {
                removeRubrique(id);
            }
        });
...
<script type="text/javascript">
    function ValiderSousDetail(id_titre, action, id_tranche) {
        var i = 1;
...
<script type="text/javascript">
    function setTranche(alimentation_id, tranche_id) {
        ...
        var montant_global=parseFloat($('#mnt_tranche').val());
        var montant_banque=parseFloat($('#alimentation_montant_' + alimentation_id).val());
        var pourcent_tranche = montant_banque / montant_global * 100;
