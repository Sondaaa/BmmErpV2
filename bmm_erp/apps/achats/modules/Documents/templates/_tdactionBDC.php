    ...
    if ($lignecaisse && $boncomm->ActionSignature() != "") {
        ...
        if (!$docs_pdcd) {
            ...
            if ($boncomm->getIdTypedoc() == 17) {
                ...
                ?>
                <a href="<?php echo url_for('documentachat/exportbccnull') . '?iddoc=' . $boncomm->getIdDocparent() . '&idbdc=' . $boncomm->getId() . '&tab=3' ?>" >
...
//die($query);
    ...
    if ($user->getIdProfil() == 28):
        ?>
        <button onclick="document.location.href = '<?php echo url_for('documentachat/deleteBDcNull') . '?id=' . $boncomm->getId() ?>'" class="btn btn-xs btn-danger"><i class="fa fa-undo"></i> Supprimer</button>   
    <?php endif; ?>
<?php else: ?>
    Annulé
<?php endif; ?>
