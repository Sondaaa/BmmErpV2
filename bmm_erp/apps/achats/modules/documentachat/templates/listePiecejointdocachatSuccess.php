<div id="sf_admin_container">
    <h1>Liste des Pieces Joints </h1>
</div>
<?php //die($id_documentachat.'rr');
$docachats = DocumentachatTable::getInstance()->findByIdTypedoc(4);
?>
        ...
        <form action="" method="post" >
            <table cellspacing="0" style="margin-bottom: 0px;">
                <tfoot>
                    <tr>
                        <td colspan="2">
                            <a href="<?php echo url_for('documentachat/listePiecejointdocachat') ?>">Effacer</a>
                ...
                <tbody>
                <tr>
                        <td><label>BCI</label></td>
                        <td>
                        <select id="id_documentachat">
                                                    <option value="0">--Sélectionnez--</option>
                                                    <?php foreach ($docachats as $doc) {?>
                                                        <option value="<?php echo $doc->getId() ?>" >
                                                        <?php echo $doc->getNumerodocachat() ?></option>
                                                    <?php }?>
                                                </select>
                        </td>
                    </tr>                    
        ...
        <div class="col-xs-12">
            ...
            <div>
                <table id="list_forme" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        <?php
include_partial('documentachat/list_piecejoint', array('pager' => $pager));
?>
                    </tbody>
                </table>
            </div>
...
<script>
    function printListDocAchats() {
        ...
        url = '<?php echo url_for('documentachat/imprimerlistePiecejoint') ?>' + url;
    ...
    function ExportListDocAchats() {
        ...
        if (url == '')
            ...
            url = url + '&idtypedoc=' + $('#id_typedoc').val();
        url = '<?php echo url_for('documentachat/exporterlisteDocumentachatAnnule') ?>' + url;
        window.open(url, '_blank');
        win.focus();
    }
