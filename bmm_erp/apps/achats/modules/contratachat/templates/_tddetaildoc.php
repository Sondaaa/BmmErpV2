<a href="#my-modal_<?php echo $boncomm->getIdTypedoc() ?>_<?php echo $boncomm->getId() ?>" role="button"  data-toggle="modal">
    <?php
    // die($boncomm->getId().'cd');
    echo $boncomm->getTypedoc() . '  ' . $boncomm->getNumero();
    ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding-top: 0px; padding-bottom: 0px;">
                <div style="margin-top: 10px; margin-bottom: 10px;">
                    ...
                    <?php } ?>
                    <?php if ($boncomm->getIdTypedoc() == 20) { ?>
                        <?php
                        $listesdocuments = Doctrine_Core::getTable('lignecontrat')
                                        ->createQuery('a')
                                        ->where('id_contrat=' . $boncomm->getId())
                                        ->orderBy('id asc')->execute();
            ...
            <div class="modal-footer">
                ...
                <a id="btnimpexpo" class="btn btn-primary pull-left" target="_blanc"
                       href="<?php echo url_for('contratachat/imprimerbContratProvisoire?iddoc=') . $boncomm->getId() ?>">
                        <i class="ace-icon fa fa-print bigger-110"></i>
                        Imprimer Contrat Provisoire : <?php echo $boncomm->getNumero() ?>
                    </a>
                <?php } ?>
                <?php if ($boncomm->getIdTypedoc() == 20) { ?> 
                <a id="btnimpexpo" class="btn btn-primary pull-left" target="_blanc"
                       href="<?php echo url_for('contratachat/imprimerbContratdefinitif?iddoc=') . $boncomm->getId() ?>">
                        <i class="ace-icon fa fa-print bigger-110"></i>
                        Imprimer Contrat Définitif : <?php echo $boncomm->getNumero() ?>
                    </a>
                <?php } ?>
