            ...
            <div class="modal-body">
                ...
                <fieldset class="col-lg-6" style="margin-bottom: 20px;">
                    ...
                    <table>
                        <tr>
                            <td>
                                <input id="iddirection" readonly="true" style="width: 70px" type="text" onkeydown="chargerRegr(event, false)" ondblclick="chargerRegr(event, true)">
                            </td>
                            <td>
                                <input id="direction" type="text" class="disabledbutton">
                            </td>
                        </tr>
                    </table>
                    ...
                    <table>
                        <tr>
                            <td>
                                <input id="idorganisme" type="text"  style="width: 70px" readonly="true" onkeydown="chargerorganisme(event, false)" ondblclick="chargerorganisme(event, true)">
                            </td>
                            <td>
                    ...
                    <table> 
                        <tr>
                            <td>
                                ...
                                <input id="idFormateur" type="text" style="width: 70px" readonly="true" onkeydown="chargerFormateur(event, false)" ondblclick="chargerFormateur(event, true)">
                            </td>
                            <td>
                    ...
                    <table> 
                        <tr>
                            <td colspan="2"><input id="anneedebut" type="text"></td>
                        </tr>
                    </table>
                    <h11><label><i>Année Fin</i></label></h11>
                    <table> 
                        <tr>
                            <td colspan="2"><input id="anneeformation" type="text"></td>
                        </tr>
                    </table>
                </fieldset>
                <fieldset class="col-lg-6">
                    ...
                    <table> 
                        <tr>
                            <td>
                                ...
                                <input id="idfonction-formation" readonly="true" style="width: 70px" type="text" onkeydown="chargerDomaine(event, false)" ondblclick="chargerDomaine(event, true)">
                            </td>
                            <td>
                                <input id="liebllefonction-formation" type="text" class="disabledbutton">
                            </td>
                        </tr>
                    </table>
                    ...
                    <table> 
                        <tr>
                            <td>
                                <input id="idunite-formation" readonly="true" style="width: 70px" type="text" onkeydown="chargerUnite(event, false)" ondblclick="chargerUnite(event, true)">
                            </td>
                            <td>
                                <input id="unite-formation" type="text" class="disabledbutton">
                            </td>
                        </tr>
                    </table>
                </fieldset>
                ...
                <div class="modal-footer">
                    <fieldset class="col-lg-12">
                        ...
                        <button type="button" value="Initialiser" class="btn btn-sm btn-primary pull-right" onclick="InitilaiserFormation()">Initialiser</button>
                    </fieldset>
                </div>
            </div>
        </div><!-- /.modal-content -->
...
<div id="my-modal-formation2" class="modal fade" tabindex="-1"> 
...
<div id="my-modalUnite-formation" class="modal fade" tabindex="-1"> 
    <?php include_partial('planing/unite_formation', array()); ?>
</div>
...
<script  type="text/javascript">
    ...
    function chargerUnite(e, dbclick) {
        ...
        if (key == 112 || dbclick) {
            $('#my-modalUnite-formation').addClass('in');
            $('#my-modalUnite-formation').css('display', 'block');
        }
    }
    function chargerorganisme(e, dbclick) {
        ...
        if (key == 112 || dbclick) {
            $('#my-modal-formation2').addClass('in');
            $('#my-modal-formation2').css('display', 'block');
        }
    }
    ...
    function chargerrubrique(e, dbclick) {
        ...
        if (key == 112 || dbclick) {
            if (($('#idfonction-formation').val() != "")) {
                $.ajax({
                    url: '<?php echo url_for('planing/chargerRubriquePardomaine()') ?>',
                    data: 'idd=' + $('#idfonction-formation').val(),
    ...
    function  filtrerAll() {
        $.ajax({
            ...
            data: 'iddebut=' + $('#anneedebut').val() +
                    ...
                    '&idunite=' + $('#idunite-formation').val() +
                    '&idregr=' + $('#iddirection').val() +
    ...
    function  filtrer()
    {
        if (($('#anneedebut').val() != "") && ($('#anneeformation').val() != "") && ($('#idunite-formation').val() != ""))
        {
            $.ajax({
                url: '<?php echo url_for('planing/chargerAgnetsParAnneeDebutFinUnite()') ?>',
                data: 'idd=' + $('#anneedebut').val() + '&idfin=' + $('#anneeformation').val() + '&idunite=' + $('#idunite-formation').val(),
                success: function (data) {
        ...
        else if (($('#anneedebut').val() != "") && ($('#idunite-formation').val() != ""))
        {
            $.ajax({
                url: '<?php echo url_for('planing/chargerAgnetsParAnneeDebutUnite()') ?>',
                data: 'idd=' + $('#anneedebut').val() + '&idunite=' + $('#idunite-formation').val(),
                success: function (data) {
        ...
        else if (($('#anneeformation').val() != "") && ($('#idunite-formation').val() != ""))
        {
            $.ajax({
                url: '<?php echo url_for('planing/chargerAgnetsParAnneeFinUnite()') ?>',
                data: 'idanne=' + $('#anneeformation').val() + '&idunite=' + $('#idunite-formation').val(),
                success: function (data) {
        ...
        else if (($('#idunite-formation').val() != "") && ($('#iddirection').val() != ""))
        {
            $.ajax({
                url: '<?php echo url_for('planing/chargerAgnetsParUniteRegroupement()') ?>',
                data: 'idd=' + $('#idunite-formation').val() + '&idregr=' + $('#iddirection').val(),
                success: function (data) {
        ...
        else if (($('#idunite-formation').val() != "") && ($('#idorganisme').val() != ""))
        {
            $.ajax({
                url: '<?php echo url_for('planing/chargerAgnetsParUniteOrganisme()') ?>',
                data: 'idd=' + $('#idunite-formation').val() + '&idorganisme=' + $('#idorganisme').val(),
                success: function (data) {
        ...
        else if (($('#idunite-formation').val() != "") && (($('#idF').val() != "")))
        {
            $.ajax({
                url: '<?php echo url_for('planing/chargerAgnetsParUniteFormateur()') ?>',
                data: 'idd=' + $('#idunite-formation').val() + '&idF=' + $('#idF').val(),
                success: function (data) {
            ...
            if (($('#idunite-formation').val() != ""))
            {
                $.ajax({
                    url: '<?php echo url_for('planing/chargerAgnetsParUnite()') ?>',
                    data: 'idd=' + $('#idunite-formation').val(),
                    success: function (data) {
