                ...
                <tr>
                    ...
                    <?php if (trim($rubrique->getTitrebudjet()->getTypebudget()) != 'Prototype'): ?>
                        <?php if (strpos(trim($typebudget), "Direction") === false && strpos(trim($typebudget), "Global") === false): ?>
                            <th style="width: 10%">MONTANT TRANSFERT <?php echo trim($rubrique->getTitrebudjet()->getTypebudget()); ?></th>
                        <?php endif; ?>
    ...
    <?php if ($rubrique->getTitrebudjet()->getEtatbudget() == 2): ?>
        <?php if (trim($rubrique->getTitrebudjet()->getTypebudget()) != "Prototype"): ?>
            ...
            <td style="text-align: center; width: 13%">
                <span class="btn btn-success btn-xs" name="btn_edit_table" onclick="setTableAnnexe('<?php echo $id; ?>', '')" <?php if ($sous_rubriques->count() != 0): ?>style="display: none;"<?php endif; ?>><i class="ace-icon fa fa-list bigger-110 icon-only"></i></span>
            </td>
        <?php endif; ?>
    <?php else: ?>
        <?php if ($rubrique->getTitrebudjet()->getEtatbudget() == 1): ?>
