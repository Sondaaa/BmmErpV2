...
<script>
    var footer = '';
<?php if ($contratprofisoire->haveToPaginate()): ?>
        footer = '<tr>' +
                ...
                '<ul class ="pagination">' +
    <?php if ($contratprofisoire->getPage() == 1): ?>
    ...
    <?php else: ?>
            '<li class ="paginate_button previous" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_previous"> <a onclick="goPageContratProviAnnule(\'1\')"> <i class="ace-icon fa fa-angle-double-left"></i> Première </a></li>' +
                    '<li class ="paginate_button previous" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_previous"> <a onclick="goPageContratProviAnnule(\'<?php echo $contratprofisoire->getPreviousPage() ?>\')"> <i class="ace-icon fa fa-angle-left"></i> Précédente </a></li>' +
    <?php endif; ?>
    <?php foreach ($contratprofisoire->getLinks() as $page): ?>
        <?php if ($page == $contratprofisoire->getPage()): ?>
                '<li class ="paginate_button active" aria-controls ="dynamic-table" tabindex ="0"> <a href = "#"> <?php echo $page ?> </a></li>' +
        <?php else: ?>
                '<li class ="paginate_button" aria-controls ="dynamic-table" tabindex ="0"> <a onclick="goPageContratProviAnnule(\'<?php echo $page ?>\')"> <?php echo $page ?> </a></li>' +
        <?php endif; ?>
    <?php endforeach; ?>
    <?php if ($contratprofisoire->getPage() == $contratprofisoire->getLastPage()): ?>
    ...
    <?php else: ?>
            '<li class ="paginate_button next" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_next"> <a onclick="goPageContratProviAnnule(\'<?php echo $contratprofisoire->getNextPage() ?>\')"> Suivante <i class="ace-icon fa fa-angle-right"></i> </a></li>' +
                    '<li class ="paginate_button next" aria-controls ="dynamic-table" tabindex ="0" id ="dynamic-table_next"> <a onclick="goPageContratProviAnnule(\'<?php echo $contratprofisoire->getLastPage() ?>\')"> Dernière <i class="ace-icon fa fa-angle-double-right"></i> </a></li>' +
    <?php endif; ?>
        '</ul>' +
                '</div>' +
                '</div>' +
...
<?php else: ?>
        footer = '<tr>' +
                ...
                '</tr>';
<?php endif; ?>
    $('#contrat_prov_annule > tfoot').html(footer);
</script>

<style>

    #contrat_prov_annule tbody td{vertical-align: middle;}
