<?php if ($agents->count() != 0): ?>
    <script type="text/javascript">
        $('#container_echelle').highcharts({
            ...
            title: {
                text: 'Statistique des Agents Par Echelle  :'
            },
            tooltip: {
            ...
            series: [{
                    type: 'pie',
                    name: 'Statistique Par Echelle',
                    data: [
    <?php for ($i = 0; $i < sizeof($agents); $i++): ?>
                            ['Echelle :<?php echo $agents[$i]['echelle'] ?>', <?php echo $agents[$i]['nbragents'] ?>],
    <?php endfor; ?>
                    ]
                }]
        });</script>
