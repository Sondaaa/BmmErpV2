<?php if ($agents->count() != 0): ?>
    <script type="text/javascript">
        $('#container_fonction').highcharts({
            ...
            title: {
                text: 'Statistique des Agents Par Fonction  :'
            },
            tooltip: {
            ...
            series: [{
                    type: 'pie',
                    name: 'Statistique Par Fonction',
                    data: [
    <?php for ($i = 0; $i < sizeof($agents); $i++): ?>
                            ['Fonction :<?php echo $agents[$i]['fonction'] ?>', <?php echo $agents[$i]['nbragents'] ?>],
    <?php endfor; ?>
                    ]
                }]
        });</script>
