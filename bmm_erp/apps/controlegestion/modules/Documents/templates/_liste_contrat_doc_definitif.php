<div class="row">
    <div class="col-xs-12">
        ...
        <div class="table-header">
            ...
            <a target="_blanc" href="<?php echo url_for('Documents/ImprimerListeDocContratFournisseurProvisoire?' . $str .'&id_type=20') ?>" class="btn btn-sm btn-success" style="float: right; padding: 5px 9px;">
                    ...
                    <table id="list_doc" class="table table-bordered table-hover" cellspacing="0">
                        ...
                        <tbody>
                            <?php if (count($pager) == 0): ?>
                                <tr>
                                    <td style="text-align:center; font-weight: bold; font-size: 16px !important;" colspan="9">Pas de Contrats Definitifs</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($pager->getResults() as $doc): ?>
                                    <tr>
                                        <td><?php
                                            echo $doc->getContratachat()->getNumero();
                                            //include_partial('tddetaildoc', array('boncomm' => $doc)) 
                                            ?></td>
                                        <td><?php echo $doc->getContratachat()->getReference(); ?></td>                                        
                                        ...
                                        <td>
                                            ...
                                            if ($documents_parent->count() != 0) {
                                                foreach ($documents_parent as $document) {
                                                    ...
                                                    echo $doc_achat->getNumerodocumentachat();
                                                }
                                            } else
                                            // include_partial('tddetaildoc', array('boncomm' => $doc->getDocumentparent()));
                                                echo $doc->getDocumentparent()->getNumerodocumentachat();
                                            ?>
                                        </td>
                                        ...
                                        <td style="text-align: center;">
                                            <a target="_blanc" href="<?php echo url_for('documentachat/showdocumentContratDef?iddoc=') . $doc->getId() ?>" class="btn btn-xs btn-primary">
                                                <i class="ace-icon fa fa-eye bigger-110"></i>
                                                <span class="bigger-110 no-text-shadow">Détails</span>
                                            </a>
                                            <?php if ($doc->getIdTypedoc() == 17): ?>
                                                <a target="_blanc" href="<?php echo url_for('documentachat/imprimerBDCProvisoire?iddoc=') . $doc->getId() ?>" class="btn btn-xs btn-success">
                                                    <i class="ace-icon fa fa-print bigger-110"></i>
                                                    Exporter PDF
                                                </a>
                                            <?php endif; ?>
