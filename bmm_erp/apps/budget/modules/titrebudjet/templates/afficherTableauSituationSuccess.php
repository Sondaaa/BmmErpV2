                                ...
                                <?php foreach ($listes as $liste) :
                                    ...
                                    <tr>
                                        <td class="fixed-side" scope="col">
                                            <b style="<?php if ($type_montant == 0) : ?>color: #0066CC;<?php else : ?>color: #AA3319;<?php endif; ?>">
                                                <?php echo $liste->getRubrique()->getCode(); ?> : </b> <?php echo $liste->getRubrique()->getLibelle(); ?>
                                        </td>
                                        <?php for ($i = $annee_debut; $i <= $annee_fin; $i++) : ?>
                                            <?php
                                            ...
                                            ?>
                                            <td><input onchange="saveMontant('<?php echo $liste->getId(); ?>', '<?php echo $i; ?>', '<?php echo $type_montant; ?>')" type="text" class="align-center" style="max-width: 130px;" id="<?php echo $liste->getId() . '_' . $i; ?>" value="<?php echo $montant; ?>" placeholder="<?php echo trim($liste->getRubrique()->getCode()) . ': => ' . $i; ?>" /></td>
                                        <?php endfor; ?>
                                    </tr>
                                    <?php foreach ($sous_listes as $sous_liste) : ?>
                                        <tr>
                                            <td class="fixed-side" scope="col">
                                                <b style="<?php if ($type_montant == 0) : ?>color: #0066CC;<?php else : ?>color: #AA3319;<?php endif; ?>">
                                                    <?php echo $sous_liste->getRubrique()->getCode(); ?> : </b> <?php echo $sous_liste->getRubrique()->getLibelle(); ?>
                                            </td>
                                            <?php for ($i = $annee_debut; $i <= $annee_fin; $i++) : ?>
                                                <?php
                                                $situationcumulee = SituationcumuleeTable::getInstance()->findOneByIdLigprotitreAndAnneesAndMois($sous_liste->getId(), $i, 12);
                                                $montant = '';
                                                if ($situationcumulee) {
                                                    if ($type_montant == 0)
                                                        $montant = $situationcumulee->getMntEngagement();
                                                ...
                                                ?>
                                                <td><input onchange="saveMontant('<?php echo $sous_liste->getId(); ?>', '<?php echo $i; ?>', '<?php echo $type_montant; ?>')" type="text" class="align-center" style="max-width: 130px;" id="<?php echo $sous_liste->getId() . '_' . $i; ?>" value="<?php echo $montant; ?>" placeholder="<?php echo trim($sous_liste->getRubrique()->getCode()) . ': => ' . $i; ?>" /></td>
                                            <?php endfor; ?>
                                        </tr>
