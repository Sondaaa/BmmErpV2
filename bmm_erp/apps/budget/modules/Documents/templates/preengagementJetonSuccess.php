...
if ($piece_jeton) {
    $idpiece_jeton = $piece_jeton->getId();
}
...
if ($form_jeton && $form_jeton->getObject()->getIdBudget()) {
    $ligne = LigprotitrubTable::getInstance()->findOneById($form_jeton->getObject()->getIdBudget());
}
?>
    ...
    <div id="sf_admin_content">
        <div class="col-sm-12">
            <div class=" tab-content">
                ...
                <?php if ($documentachatjeton) { ?>
                    <p>
                        <a target="__blanc" href="<?php echo url_for('documentachat/ImprimerdocachatJeton?iddoc=' . $documentachatjeton->getId()) ?>">
                            <i class="fa fa-eye"></i> <?php echo $documentachatjeton->getTypedoc() ?> N°:<?php echo $documentachatjeton->getNumerodocachat() ?>
                        </a>
                    </p>
                        ...
                        <table>
                            <tbody>
                                ...
                                <tr class="disabledbutton">
                                    ...
                                    <td>
                                        <ul>
                                            <li>Alloué: <input type="text" class="form-control" value="<?php
                                                if ($ligne && $ligne->getIdTitre()) {
                                                    echo number_format($ligne->getMnt(), 3, ',', ' ');
                                                }
                                                ?>" id="mnt"></li>
                                            <li>Bloqué: <input type="text" class="form-control" value="<?php
                                                if ($ligne && $ligne->getIdTitre()) {
                                    ...
                                    <td>
                                        <ul>
                                            <li>Définitive: <input type="text" class="form-control" value="<?php
                                                if ($ligne && $ligne->getIdTitre() && $ligne->getMntengage()) {
                                                    echo number_format($ligne->getMntengage(), 3, ',', ' ');
                                                }
                                                ?>" id="credit"></li>
                                            <li>Provisoire: <input type="text" class="form-control" value="<?php
                                                if ($ligne && $ligne->getIdTitre()) {
                                                    echo number_format($ligne->getMntprovisoire(), 3, ',', ' ');
                                                }
                                                ?>" id="creaditporv"></li>
                                                                   <?php $total = $ligne->getMntprovisoire() + $ligne->getMntengage(); ?>
                                            <li>Total: <input type="text" class="form-control" value="<?php
                                                if ($ligne && $ligne->getIdTitre()) {
...
//                                        echo $piece->getDescription();
        ...
        <!--                            <input type="button" value="Valider Engagement" ng-click="ValiderEngagement(<?php // echo $documentachat->getId()   ?>,<?php // echo $form->getObject()->getId()   ?>)">-->
                    ...
                    if (!$form->getObject()->isNew() && $form->getObject()->getMnt() && $doc->getAnnule() != '1') {
                        ?>
                        <input type="button" value="Annulation l'Engagement" ng-click="AnnulerEngagementDef(<?php echo $documentachat->getId() ?>,<?php echo $form->getObject()->getId() ?>)" class="btn-danger">
                    <?php } ?>
        ...
        <div class="col-sm-6">
            <div class=" tab-content">
                ...
                <fieldset <?php
                if ($piece_jeton && $piece_jeton != 0 && $piece_jeton != null) {
                    echo 'class="disabledbutton"';
                }
                ?>>
                    <legend> <?php
                        if ($piece_jeton) {
                            if (!$form_jeton->getObject()->isNew()) {
                                echo $form_jeton->getObject()->getTypedocbudget();
                            }
                        } else {
                            $form_jeton = new DocumentbudgetForm();
                        }
                        ?></legend>
                    <table>
                        <tbody>
                            <tr class="disabledbutton">
                                ...
                                <td colspan="4">
                                    ...
                                    if ($form_jeton->getObject()->isNew()) {
                                        echo '1';
                                        $documentachat = $documentachatjeton;
                                    }
                                    ?>">
                                           <?php echo $form_jeton['id_type']->renderError() ?>
                                           <?php echo $form_jeton['id_type'] ?>
                                </td>
                            </tr>
                            <tr class="disabledbutton">
                                <td><label>Numéro</label></td>
                                <td colspan="4">
                                    <?php echo $form_jeton['numero']->renderError() ?>
                                    <?php
                                    if ($form_jeton->getObject()->isNew()) {
                                        echo $form_jeton['numero']->render(array('value' => $form_jeton->getObject()->NumeroSeqDocumentAchat(3)));
                                    } else {
                                        echo $form_jeton['numero']->render(array('value' => $form_jeton->getObject()->getNumerodocachat()));
                                    }
                                    ?>
                                </td>
                            </tr>
                            <tr>
                                <td><label>Date Création</label></td>
                                <td>
                                    <?php echo $form_jeton['datecreation']->renderError() ?>
                                    <?php echo $form_jeton['datecreation']; ?>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
//                                        echo $piece->getDescription();
                ...
                <fieldset>
                    ...
                    <?php if ($doc_achat->getIdTypedoc() == 7) { ?>
                        ...
                        <?php } ?>
                    <?php } ?>
                    <?php if ($doc_achat->getIdTypedoc() == 2 || $doc_achat->getIdTypedoc() == 17 || $doc_achat->getIdTypedoc() == 7 || $doc_achat->getIdTypedoc() == 18) { ?>
                        <?php if ($idpiece == 0) { ?>
                            <input type="button" value="Mettre à ajour" ng-click="AjouterPreengagement(<?php echo $documentachat->getId() ?>)">
                            <?php
                        }
                        ...
                        $doc = DocumentbudgetTable::getInstance()->findById($piece->getIdDocumentbudget())->getLast();
                        //$documentachat->ValidationEngagement() == 0 &&   
                        //die('eecd'. $idpiece .'eecccsss'..'eeee'.$doc->getAnnule());
                        if ($idpiece != 0 && !$form_jeton->getObject()->getId() && $doc->getAnnule() == 1) {
                            // && !$form_jeton->getObject()->getMnt()
                            ?>
                            <input type="button" value="Valider Engagement Jeton" ng-click="ValiderEngagementJeton(<?php echo $bcej->getId() ?>,<?php echo $doc_achat->getId() ?>)">
                        <?php } ?>
                    <?php } ?>
                    <?php ?>
                    <?php if (!$form->getObject()->isNew() && $form->getObject()->getMnt()) { ?>
                        <a class="btn btn-white btn-default "
                           target="__blanc" href="<?php echo url_for('Documents/Imprimerprovisoire') . '?idfiche=' . $form_jeton->getObject()->getId() . '&iddoc=' . $documentachat->getId() ?>">Exporter Pdf & Impression <?php echo $form->getObject()->getTypedocbudget() ?></a>
                       <?php } ?>
                </fieldset>
            </div>
        </div>
