        ...
        <form action="" method="post">
            <table cellspacing="0" style="margin-bottom: 0px;">
                <tfoot>
                    <tr>
                        <td colspan="2">
                            <a href="<?php echo url_for('Documents/indexfrsBcicontrat') ?>">Effacer</a>
...
//                                die(($boncomm->getDocumentparent()->getLigavisdoc()->getFirst()->getId()).'mp');

                                $contrat = $boncomm->getContratachat();
                                $id_doc = $boncomm->getIdDocparent();
                                $docachat = DocumentachatTable::getInstance()->findOneById($id_doc);

                                if ($docachat->getIdDocparent() != null) {
                                    $docachat_pro = DocumentachatTable::getInstance()->findOneById($docachat->getIdDocparent());
                                }
                                //die(sizeof($docachat->getLigavisdoc()) . 'mp' . $docachat->getId() . 'dzd' . sizeof($docachat_pro->getLigavisdoc()) . 'fv' . $docachat_pro->getId());
                                if (sizeof($docachat->getLigavisdoc()) >= 1)
                                    $ligprotitrub = $docachat->getLigavisdoc()->getFirst()->getLigprotitrub();

                                elseif ($docachat->getDocumentparent()->getLigavisdoc()->getFirst())
                                    $ligprotitrub = $docachat->getDocumentparent()->getLigavisdoc()->getFirst()->getLigprotitrub();
                                //die($contrat->getDocumentachat()->getFirst()->getId() . 'f');
                                ?>
                                <tr>
                                    <td style="text-align: center;"><?php //die( $boncomm->getContratachat()->getId().'fv');        ?>
                                        <input type="checkbox" id="check_<?php echo $boncomm->getId() ?>"  ng-click="ValiderChoisirContrat('<?php echo $boncomm->getId() ?>', '<?php echo $boncomm->getContratachat()->getMontantcontrat() ?>', '<?php echo $ligprotitrub->getId() ?>', '<?php echo $ligprotitrub->getIdTitre() ?>')">
                                        <?php
                                        //if ($docachat->getDocumentparent())
                                        //if ($docachat->getDocumentparent()->getLigavisdoc()->getFirst()): 
                                        ?>

    <?php // endif;   ?>   
                                    </td>
                                    <td style="text-align: center;"><?php echo date('d/m/Y', strtotime($boncomm->getContratachat()->getDatecreation())) ?></td> 
                                    <td style="text-align: center;">
                                        <a href="#my-modal<?php echo $boncomm->getId() ?>" role="button"  data-toggle="modal">
    <?php echo $contrat->getReference() . ' N° ' . $contrat->getNumero() ?>
                                        </a>
...
//                                                    }
                                                    ...
                                                    <div class="modal-header">
                                                        ...
                                                        <!--                                                        <h3 class="smaller lighter blue no-margin" style="text-align: left;">
                                                                                                                    Détail <?php // echo $numero . ' ' . $boncomm         ?></h3>-->
                                                    </div>
                                                    <div class="modal-body">
                                                        <?php
                                                        if ($contrat->getId() != '' && $contrat->getId() != null)
//                                                        {
////                                                        echo html_entity_decode($boncomm->ReadHtmlBondeponseForm());
//                                                        } else {
                                                            echo html_entity_decode($boncomm->ReadHtmlBCIContrat($contrat->getId()));
//                                                        }
                                                        ?>
                                                        <?php
                                                        // echo(sizeof($boncomm->getContratachat()).'rrrrrrrrrr'.$boncomm->getContratachat()->getId());
//                                                        if ($boncomm->getIdTypedoc() == 19 && sizeof($boncomm->getContratachat())>=1) { 
                                                        ?>
                                                        <?php
//                                                            $listesdocuments = Doctrine_Core::getTable('lignecontrat')
//                                                                            ->createQuery('a')
//                                                                            ->where('id_contrat=' . $boncomm->getContratachat()->getId())
//                                                                            ->orderBy('id asc')
//                                                                    ->execute()
//                                                                    ;
////                                                            die($listesdocuments);
...
//                                    $numero = str_replace(array('à', 'á', 'â', 'ã', 'ä', 'ç', 'è', 'é', 'ê', 'ë', 'ì', 'í', 'î', 'ï', 'ñ', 'ò', 'ó', 'ô', 'õ', 'ö', 'ù', 'ú', 'û', 'ü', 'ý', 'ÿ'), array('À', 'Á', 'Â', 'Ã', 'Ä', 'Ç', 'È', 'É', 'Ê', 'Ë', 'Ì', 'Í', 'Î', 'Ï', 'Ñ', 'Ò', 'Ó', 'Ô', 'Õ', 'Ö', 'Ù', 'Ú', 'Û', 'Ü', 'Ý'), $numero);
                                    ?>
                                    <td style="text-align: center;">
                                        <a href="#bci_model<?php echo $boncomm->getId() ?>" role="button" data-toggle="modal"><!--#my-modal<?php // echo $boncomm->getId()      ?>-->
    <?php echo $boncomm->getNumerodocumentachat() ?>
                                        </a>
                                        <div id="bci_model<?php echo $boncomm->getId() ?>" class="modal fade" tabindex="-1">
                                            <div class="modal-dialog" style="width: 84%;">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                                                        <h3 class="smaller lighter blue no-margin" style="text-align: left;">Détail 
                                                            <?php // $boncomm->getNumerodocumentachat()      ?> 
    <?php // echo $boncomm->getDocumentparent()->getNumerodocachat();            ?> </h3>
                                                    </div>
                                                    <div class="modal-body">
    <?php echo html_entity_decode($boncomm->getBonCommandeInterneContratForm()); ?>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button class="btn btn-sm btn-danger pull-right" data-dismiss="modal">
                                                            <i class="ace-icon fa fa-times"></i>
                                    ...
                                    <td style="text-align: center;">
                                            <?php if ($docachat->getDocumentparent()) if ($docachat->getDocumentparent()->getLigavisdoc()->getFirst()): ?>
    ...
    <?php  endif; ?>
                                    ...
                                    <td><?php echo $contrat->getFournisseur()->getRs(); ?></td> 
    <!--                                    <td style="width: 9%; text-align: right;"><?php // if ($boncomm->getMht()) echo number_format($boncomm->getMht(), 3, '.', ' ');               ?></td> 
                                    ...
                                    <td style="width: 9%; text-align: right;"><?php echo number_format($contrat->getMontantcontrat(), 3, '.', ' '); ?></td> 
                                </tr>
                                <?php
                            }
                            ?>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h3 class="smaller lighter blue no-margin">
                                <?php
                                if ($boncomm->getIdTypedoc() != 19) {
                                    echo 'Détail ' . $boncomm->getNumerodocachat();
                                } else
                                    $boncomm->getContratachat()->getReference() . ' N° ' . $boncomm->getContratachat()->getNumero();
                                ?>
                            </h3>
                        </div>
                        <div class="modal-body">
...
<?php } ?>
                        ...
                        <div class="modal-footer">
                            <button ng-click="AjouterParDocPreengagementContrat()" class="btn btn-sm btn-success pull-right"  style="margin-left: 1%">
