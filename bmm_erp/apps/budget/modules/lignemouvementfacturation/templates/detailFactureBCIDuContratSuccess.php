    ...
    <div class="panel-body">
        ...
        <div class="tab-content">
            <div class="tab-pane fade active in" id="accuelbci">
                <div class="row">
                    <div class="col-lg-12">
                        <?php echo html_entity_decode($documentachat->ReadHtmlFactureImressionBCIContratFacturation($id)); ?> 
            ...
            <div class="tab-pane" id="engagement">
                <?php
                $doc_pa = DocumentachatTable::getInstance()->findOneByIdDocparent($documentachat->getId());
                $piecejointbudget = PiecejointbudgetTable::getInstance()->findOneByIdDocachat($doc_pa->getId());
                ?>
                <?php
                // die(sizeof($piecejointbudget) . 'mp' . $documentachat->getId());
                $document_budget = $piecejointbudget->getDocumentbudget();
                ?>
                <?php
                //Charger facture d'aprés BCE Système
                $id_doc_achat = $mvt->getDocumentachat()->getId();
                $doc_achat = DocumentachatTable::getInstance()->find($id_doc_achat);
//                $id_parent = $doc_achat->getIdDocparent();
//                $doc_parent = DocumentachatTable::getInstance()->find($id_parent);
//                $id_doc_parent = $doc_parent->getIdDocparent();
//                die($id_doc_parent.'mp');
                $facture = DocumentachatTable::getInstance()->findOneByIdDocparentAndIdTypedoc($id_doc_achat, 15);
               
                if (!$facture) { 
                    //Carger facture d'aprés BCEJ
                    $bcej = DocumentachatTable::getInstance()->findOneByIdDocparentAndIdTypedoc($id_doc_achat, 16);
                    if ($bcej)
                        $facture = DocumentachatTable::getInstance()->findOneByIdDocparentAndIdTypedoc($id_doc_achat, 15);
                    else
                        $facture = null;
                                                            ...
                                                            <tbody>
                                                                <tr>
                                                                    ...
                                                                    <td style="width: 70%;"><?php
                                                                        ...
                                                                        //date('Y', strtotime($document_budget->getDatecreation())); 
                                                                        ?></td>                    
                                                                </tr>
                                                                <tr>
                                                        ...
                                                        <div class="col-lg-12">
                                                            <div class="content">
                                                                ...
                                                                <?php if (trim($documentachat->getEtatdocachat()) != "Annulé(e)"): ?>
                                                                    <button id="valide_button" class="btn btn-sm btn-success" onclick="validerDocumentBCIContrat()">
...
<script  type="text/javascript">
    ...
    function validerDocumentBCIContrat() {
        $.ajax({
            url: '<?php echo url_for('documentbudget/validerOrdonnanceBCIcontrat') ?>',
            data: 'idbudget=<?php echo $document_budget->getLigprotitrub()->getId(); ?>' +
                    '&iddoc=<?php echo $doc_pa->getId(); ?>' +
                    '&idtype=2' +
                    '&idpreengagement=<?php echo $document_budget->getId(); ?>' + '&id_mvt=<?php echo $id; ?>',
            success: function (data) {
                $('#valide_button').hide();
                bootbox.dialog({
