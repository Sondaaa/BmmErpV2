<div id="sf_admin_container"   >
    ...
    ?>
    <div id="sf_admin_content" ng-controller="myCtrlbudget" ng-init="InitialiserBudget()">
        <div style=" position: absolute; float: right; margin-left: 80%; margin-top: 1%;">
            <?php if ($documentachat->getIdTypedoc() != 9): ?>
                <?php if ($documentachat->getLigavisdoc()->count() == 0): ?>
                    <table>
                        <thead>
                            <tr>
                        ...
                        foreach ($aviss as $avis) {
                            ...
                            <tr>
                                <?php if (strpos($avis->getLibelle(), ":") == 0): ?>
                                    <?php if (trim($avis->getLibelle()) == 'Crédit Insuffisant' || trim($avis->getLibelle()) == 'Crédit Disponible'): ?>
                                        <td>
                                        ...
                                        <td>
                                            <input class="disabledbutton" name="avis_checkbox" type="checkbox" <?php
                                            if ($lgavis) {
                                                $count_checked++;
                                                echo 'checked="true"';
                                            }
                                            ?> id="check1_<?php echo $avis->getId() ?>">
                                        </td>
                                    <?php else: ?>
                                        <td><?php echo $avis->getLibelle(); ?></td>
                                        <td>
                                            <input style="cursor: pointer;"  ng-click="ValiderChoix('<?php echo $avis->getId() ?>', '<?php echo $documentachat->getId() ?>')" name="avis_checkbox" type="checkbox" <?php
                                            if ($lgavis) {
                                                $count_checked++;
                                                echo 'checked="true"';
                                            }
...
//                                                $annees = date('Y');
                        ...
                        <tr>
                            <?php if (strpos($avis->getLibelle(), ":") == 0): ?>
                                ...
                                <td>
                                    ...
                                    <input name="avis_checkbox" ng-controller="CtrlFormEngagement" ng-click="ValiderChoix('<?php echo $avis->getId() ?>', '<?php echo $documentachat->getId() ?>')" type="checkbox" <?php
                                    if ($lgavis) {
                                        $count_checked++;
                                        echo 'checked="true"';
                                    }
        ...
        <?php if ($documentachat->getIdEtatdoc() != 9 && $documentachat->getLigavisdoc()->count() == 0): ?>
        ...
        <?php else: ?>
            <fieldset id="zone_avis_budgetaire">
                <legend>Avis Budgétaire
                    <?php
                    // die($documentachat->getIdEtatdoc().'mp');
                    if ($documentachat->getIdEtatdoc() == 34):
                        ?>
...
//                                                $annees = date('Y');
            ...
            <?php if ($ligavissig->count() == 0 && $documentachat->getIdEtatdoc() == 9 && $documentachat->getLigavisdoc()->count() != 0): ?>
