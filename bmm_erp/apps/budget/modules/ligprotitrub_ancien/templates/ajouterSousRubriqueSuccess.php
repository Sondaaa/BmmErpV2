...
<tr id="tr_<?php echo $index . '_' . $id; ?>" name="tr_rubrique">
            ...
            <thead>
                <tr>
                    ...
                    <?php if (trim($titre->getTypebudget()) != 'Prototype'): ?>
                        ...
                        <th style="width: 10%">MONTANT EN CAISSER</th>
    ...
    <?php if (trim($titre->getTypebudget()) != 'Prototype'): ?>
        ...
        <input type="text" class="align_right" readonly="true" name="credit_encaissier" parent_line="<?php echo $index; ?>" id="credit_encaissier_<?php echo $index . '_' . $id; ?>" value="<?php echo number_format($montant_encaissier, 3, '.', ''); ?>" style="max-width: 130px; width: 100%;">
    ...
    <?php else: ?>
        <td style="text-align: center; width: 13%">
            ...
            <span class="btn btn-xs btn-danger btn-xs" name="btn_remove_rubrique" onclick="removeRubrique('<?php echo $index . '_' . $id; ?>', '<?php echo $index; ?>')"><i class="ace-icon fa fa-remove bigger-110 icon-only"></i></span>
        </td>
    <?php endif; ?>
</tr>
