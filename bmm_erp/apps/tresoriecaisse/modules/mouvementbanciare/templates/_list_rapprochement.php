                                ...
                                <?php foreach ($mouvements as $mvt): ?>
                                    <tr id="ligne_<?php echo $mvt->getId(); ?>">
                                        ...
                                        <td style="text-align: center; vertical-align: middle;">
                                            <?php if ($mvt->getAnnule() == false): ?>
                                                <input id="<?php echo $mvt->getId(); ?>" type="checkbox" data-mouvement="1" value="ligne_<?php echo $mvt->getId(); ?>" montant_debit="<?php echo $montant_debit; ?>" montant_credit="<?php echo $montant_credit; ?>" class="list_checbox_mouvement" <?php if ($mvt->getRapproche() == true): ?>checked="true"<?php endif;?> />
                                                    <input id="<?php echo $mvt->getId(); ?>" type="button"
                                                  value="+" class="btn btn-xs btn-primary" style="margin-left: 2px" onclick="AddLigneCommision('<?php echo $mvt->getId(); ?>')" />
                                    ...
                                    <?php if ($mvt->getMntenoper() != null): ?>
                                        <tr id="ligne_bis_<?php echo $mvt->getId(); ?>">
                                            ...
                                            <td style="text-align: right;"><?php echo number_format($solde_courant, 3, '.', ' '); ?></td>
                                            <td style="text-align: center; vertical-align: middle;">
                                                <input id="<?php echo $mvt->getId(); ?>" type="checkbox" data-mouvement="0" value="ligne_bis_<?php echo $mvt->getId(); ?>" montant_debit="<?php echo $mvt->getMntenoper(); ?>" montant_credit="0" class="list_checbox_mouvement" <?php if ($mvt->getRapprochecommission() == true): ?>checked="true"<?php endif;?> />
                                            </td>
                                        </tr>
                                    <?php endif;?>
                                <?php endforeach;?>
 ...
 function  AddLigneCommision(id)
    {
            ...
            $.ajax({
                url: '<?php echo url_for('mouvementbanciare/addLigne') ?>',
                async: true,
                data: 'id=' +id ,
                success: function(data) {

                    $('#list_mouvements tbody').append(data);
    ...
    function setTotal() {
        ...
        $('#total_selected_debit').html(formatter.format(total_debit));
         reste_debit =parseFloat(parseFloat('<?php echo $total_debit; ?>') - parseFloat(total_debit));
         reste_credit =parseFloat(parseFloat('<?php echo $total_credit; ?>') - parseFloat(total_credit));
    ...
    function saveMvt(id) {console.log('id=');
        var nb_lignes = 0;
        i_ligne=0;
        //alert('dede'+nb_lignes);
      var  date_operation =  $('#ligne_date_' + i_ligne).val() ;
      var  nom_operation = $('#nom_operation_' + i_ligne).val() ;
      var  debit = $('#mnt_debit_' + i_ligne).val();
        // $('#list_mouvements tbody tr').each(function() {alert('dlllllede'+nb_lignes);
        //         nb_lignes++;
        //         var i_ligne = $(this).attr('index_ligne');
        //         console.log('index=' + i_ligne);
        //         date_operation = date_operation + $('#ligne_date_' + i_ligne).val() + ',,';
        //         nom_operation = nom_operation + $('#nom_operation' + i_ligne).val() + ',,';
        //         debit = debit + $('#mnt_debit' + i_ligne).val() + ',,';
            // });
                $.ajax({
                    url: '<?php echo url_for('mouvementbanciare/validerOperation') ?>',
                    data: 'date=' + date_operation +
                        '&nom=' + nom_operation +
                        '&debit=' +  debit+
                        '&id=' +  id ,
                                success: function(data) {
                            ...
                            //  $('#sf_admin_content').fadeIn();
                                    bootbox.dialog({
                                        message: "<span class='bigger-160' style='margin:20px;'> " + " Mouvemet(s) bancaire(s) créée(s) avec succès !</span>",
                                        buttons: {
                                            "button": {
                                                "label": "Ok",
                                                "className": "btn-sm"
