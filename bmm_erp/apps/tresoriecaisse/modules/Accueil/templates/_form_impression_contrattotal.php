    ...
    <div class="modal-dialog" style="width: 900px;">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="smaller lighter blue no-margin">SUIVI CONTRAT : IMPRESSION PERSONNALISEE </h4>
            </div>
            <div class="modal-body">
                <fieldset class="col-lg-12">
                    <legend>Données de Contrat<input id="base_check" name="base_check" type="checkbox" style="float: right;" onchange="checkAllBase()"></legend>
                    <table style="width: 100%">
                        <tr > 
                            <td rowspan="2"><h4 class="smaller lighter blue no-margin"><i>CONTRAT</i></h4></td>
                            <td>BCI</td>
                        ...
                        <tr> 
                            <td>Contrat Provisoire</td>
                            <td><input id="numero_contrat" name="numero_contrat" type="checkbox" style="width: 25px"></td>
                            <td>N°Contrat Définitif</td>
                            <td><input id="numero_contratdef" name="numero_contratdef" type="checkbox" style="width: 25px"></td>
                            <td>Montnt Contrat Def.</td>
                            <td><input id="montant_contratdef" name="montant_contratdef" type="checkbox" style="width: 25px"></td>
                            <td></td>
                        </tr> 
...
<script  type="text/javascript">
    function annuler() {
        $('#my-modalimpressioncontrattotal ').removeClass('in');
        $('#my-modalimpressioncontrattotal ').css('display', 'none');
    ...
    function checkAllBase() {
        if ($('input[name=base_check]').is(':checked')) {
            ...
            $("#numero_contrat").prop("checked", true);
            $("#numero_contratdef").prop("checked", true);
            $("#montant_contratdef").prop("checked", true);
            ...
            $("#numero_contrat").prop("checked", false);
            $("#numero_contratdef").prop("checked", false);
            $("#montant_contratdef").prop("checked", false);
    ...
    function printfiche() {
        ...
        if ($('input[name=numero_contrat]').is(':checked')) {
            if (url == '')
                url = '?numero_contrat=' + $('#numero_contrat').val();
            else
                url = url + '&numero_contrat=' + $('#numero_contrat').val();
            valide = true;
        }

        if ($('input[name=numero_contratdef]').is(':checked')) {
            if (url == '')
                url = '?numero_contratdef=' + $('#numero_contratdef').val();
            else
                url = url + '&numero_contratdef=' + $('#numero_contratdef').val();
            valide = true;
        }

        if ($('input[name=montant_contratdef]').is(':checked')) {
            if (url == '')
                url = '?montant_contratdef=' + $('#montant_contratdef').val();
            else
                url = url + '&montant_contratdef=' + $('#montant_contratdef').val();
            valide = true;
        }
        ...
        if (valide) {
            ...
            url = '<?php echo url_for('Accueil/imprimerFicheContratAvecChoix') ?>' + url;
