...
<script>
    ...
    function showDetail(id, id_bordereau) {
        $.ajax({
            url: '<?php echo url_for('papierordrepostal/detail') ?>',
            data: 'id=' + id + '&id_bordereau=' + id_bordereau,
            success: function (data) {
                bootbox.dialog({
                    message: data,
                    buttons:
    ...
    function annulerOrdre(id, numero) {
        bootbox.dialog({
            message: "<span class='bigger-110'>Voulez-vous supprimer cet ordre de virement : "
                    + numero + " ?</span><div style='margin-left: 10%; \n\
        margin-top: 10px;width: 75%;text-align: justify;border-left: 3px solid #bf9090;padding-left: 15px;'>Si c'est le cas, veuillez choisir entre : <ul style='font-weight: normal;margin-top: 10px;'>\n\
<li style='line-height: 25px;margin-bottom: 15px;'>\n\
La destruction physique du l'ordre de virement (cas de falsification).\n\
 <i class='ace-icon fa fa-hand-o-right bigger-110 red' style='margin-left: 20px;'>\n\
 Détruire</i></li><li style='line-height: 25px;margin-bottom: 15px;'>L'initialisation du l'ordre de virement pour l'utiliser une fois de nouveau.\n\
 <i class='ace-icon fa fa-hand-o-right bigger-110 blue' style='margin-left: 20px;'>\n\
 Initialiser</i></li><ul></div>",
            buttons:
                    {
                        "danger":
                                {
