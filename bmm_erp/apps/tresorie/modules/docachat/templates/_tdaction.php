...
<?php if ($boncomm->getEtatdocachat() == ''): ?>
    ...
    $bdcregrp_def = DocumentachatTable::getInstance()->findByIdTypedocAndIdFils(22, $boncomm->getId());
    ?>
    <?php
    if ($boncomm->getIdTypedoc() == 21 && $boncomm->getIdEtatdoc() == 62 && sizeof($bdcregrp_def) == 0) {
        // && $boncomm->getDatesignature() != null
        ?>
        <a href="<?php echo url_for('documentachat/exportbccrp') . '?iddoc=' . $boncomm->getIdDocparent() . '&idbdc=' . $boncomm->getId() . '&tab=3' ?>" >
            Exporter En BDC Regroupe. Définitif
        </a>
    <?php } ?>
    <?php
    if ($lignecaisse && $boncomm->ActionSignature() != "") {
        ...
        if (!$docs_pdcd) {
            ...
            if ($boncomm->getIdTypedoc() == 17) {
                // && $boncomm->getDatesignature() != null
                ?>
                <a href="<?php echo url_for('docachat/exportbcc') . '?iddoc=' . $boncomm->getIdDocparent() . '&idbdc=' . $boncomm->getId() . '&tab=3' ?>" >
