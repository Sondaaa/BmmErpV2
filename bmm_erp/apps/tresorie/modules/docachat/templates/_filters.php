...
<div class="sf_admin_filter col-xs-6">
  ...
  <form action="<?php echo url_for('documentachat_docachat_collection', array('action' => 'filter')) ?>" method="post">
    <table cellspacing="0">
      <tfoot>
        <tr>
          <td colspan="2">
            <?php echo $form->renderHiddenFields() ?>
            <?php echo link_to(__('Reset', array(), 'sf_admin'), 'documentachat_docachat_collection', array('action' => 'filter'), array('query_string' => '_reset', 'method' => 'post')) ?>
      ...
      <tbody>
        ...
        <?php if ((isset($form[$name]) && $form[$name]->isHidden()) || (!isset($form[$name]) && $field->isReal())) continue ?>
          <?php include_partial('docachat/filters_field', array(
            'name'       => $name,
...
//        $('#documentachat_filters_datecreation_to').attr('max', max_date);
    ...
    function printListDocAchats() {
        ...
        url = '<?php echo url_for('docachat/imprimerlistedocument') ?>' + url;
    ...
    function ExportListDocAchats() {
        ...
        url = '<?php echo url_for('docachat/exporterdocumentExcel') ?>' + url;
        window.open(url, '_blank');
        win.focus();
    }
</script>