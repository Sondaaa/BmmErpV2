                            ...
                            <?php else: ?>
                                ...
                                <?php if ($quitance == 1): ?>
                                    <?php // foreach ($mouvements as $mvt): ?>
                                        <?php
                                        $document_achat = $mvt->getDocumentachat();
                                        $doc_achat_provisoire = DocumentachatTable::getInstance()->findOneByIdDocparentAndIdTypedoc($document_achat->getIdDocparent(), 17);
//                                        die($document_achat->getId().'vd');
                                        if ($document_achat) {
                                            $quitances = Doctrine_Query::create()
                                                    ->select("*")
                                                    ->from('ligneoperationcaisse d,documentachat')
                                                    ->where('d.id_docachat=documentachat.id');
                                            if (!$doc_achat_provisoire&& $document_achat)
                                                $quitances = $quitances->andwhere("d.id_docachat=" . $document_achat->getId());
                                            if ($doc_achat_provisoire)
                                                $quitances = $quitances->andwhere("((d.id_docachat=" . $document_achat->getId() . ") or "
                                                                . "(d.id_docachat=" . $doc_achat_provisoire->getId() . '))')->andWhere('d.id_docachat=documentachat.id');
                                            $quitances = $quitances->andWhere("d.etat =  'Actif'");
//                                                      ->where("f.etatfrs ='Actif'")
//                                            die($quitances.$doc_achat_provisoire);
                                            $quitances = $quitances->execute();
                                        }
//                                        else {
//                                            $quitances = null;
//                                        }
//                                        die(json_encode($quitances) . $document_achat->getId());
                                        ?>
                                        <?php foreach ($quitances as $q): ?>
                                            <tr>
                                                <td>--</td>
                                                <td style="text-align: center;"><?php echo date('d/m/Y', strtotime($q->getDateoperation())); ?></td>
                                                <td style="text-align: center;"><?php echo $q->getDocumentachat()->getNumerodocachat(); ?></td>
                                                <td style="text-align: center;">
                                                    Quittance
                                                </td>
                                                <td><?php echo $q->getCategorieoperation(); ?></td>
                                                <td><?php // echo $q->getCategorieoperation();               ?></td>
                                                <td style="text-align: right;">
                                                    <?php // if ($mvt->getCredit() != null): ?>
                                                    <?php // echo number_format($q->getMntoperation(), 3, '.', ' '); ?>
                                                    <?php // endif; ?>
                                                </td>
                                                <td style="text-align: right;">
                                                    <?php echo number_format($q->getMntoperation(), 3, '.', ' '); ?>
