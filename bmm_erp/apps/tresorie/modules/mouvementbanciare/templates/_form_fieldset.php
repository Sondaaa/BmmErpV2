<fieldset id="sf_fieldset_<?php echo preg_replace('/[^a-z0-9_]/', '_', strtolower($fieldset)) ?>">
    ...
    <?php endif; ?>
    <?php
    $entete = SocieteTable::getInstance()->find(1)->getRs();
    ?>
</fieldset>
<input type="hidden" id="typedoc" value="">
<input type="hidden" id="typebdcr" value="<?php echo $type; ?>">
<div class="row">
    <div class="col-xs-12">
        ...
        <div class="widget-box">
            ...
            <div class="widget-body" >
                <div class="widget-main" style="padding-bottom: 0px;" ng-init="InitiliserCaisseanque('<?php echo $id; ?>')">
                    ...
                    if ($id) {
                        $document_par = DocumentachatTable::getInstance()->find($id);
//                        die($document_par->getIdDocparent().'frfe');
                        $document = DocumentachatTable::getInstance()->findOneById($document_par->getIdFils());
                        $documentbudget = $document_par->getPiecejointbudget()->getFirst()->getDocumentbudget()->getLigprotitrub()->getId();
                    
                     
                     $lignecaisse = Doctrine_Core::getTable('lignebanquecaisse')->findByIdBudget($documentbudget);
//                        die($lignecaisse->getId()) . "depp");
                    }
//                    die($lignecaisse->getIdCaissebanque().'m');
//                    echo($lignecaisse->getIdCaissebanque().' mp '.$documentbudget)
                    ?>
                    <table>
                        <tr>
                            <td>Caisse</td>
                            <td <?php if ($id != null) { ?>class="disabledbutton"<?php } ?>>
                                ...
                                <!--<input type="text" id="mouvementbanciare_id_banque"  value="<?php // echo $documentbudget;                       ?>">name="mouvementbanciare[id_banque]" id="mouvementbanciare_id_banque"-->
                                ...
                                <select name="mouvementbanciare[id_banque]" id="mouvementbanciare_id_banque" >
                                    <?php foreach ($caisses as $caisse):
                                        ...
                                        <option <?php
                                        if ($id != null) {
                                            if ($lignecaisse->getLast()->getIdCaissebanque() == $caisse->getId()):
                                                ?> 
                                                    selected="true"<?php
                                                endif;
                                            }
        ...
        <div class="widget-box" >
            ...
            <div class="widget-body">
                <div class="widget-main" style="padding-bottom: 0px; width: 100%; overflow: auto; min-height: 500px">
                    <table id="idformulaire">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php if ($type == "BDCG"): ?>
                                    Factures &&&&  Bénéficiaire
                                    <div id="beneficiaire_input">
                                        <input type="text" id="factures" >
                                        <input type="text" id="refbeni" >
                                    </div>
                                <?php else: ?> 
                                    Bénéficiaire
                                    <div id="beneficiaire_input">
                    ...
                    <?php if ($type == 'BDCG') { ?>
                        <a ng-click="SaveOperationsBDCReg()" class="btn btn-success" style="float: right; padding: 4px 12px; padding-top: 3px;"><i class="ace-icon fa fa-save align-center bigger-110"></i> Valider</a>
                    <?php } else { ?>
                    ...
                    <?php if ($type == 'BDCG') { ?>
                        <a title="Ajouter à la fin." data-rel="tooltip" ng-click="PushNewLigneBDCG()" class="btn btn-info" style="float: right; padding: 4px 12px; padding-top: 3px;"><i class="ace-icon fa fa-arrow-down align-center bigger-110"></i> Ajouter Ligne</a>
                    <?php } else { ?>
