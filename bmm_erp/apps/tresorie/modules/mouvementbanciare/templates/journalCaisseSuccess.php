...
<div id="sf_admin_container" ng-controller="CtrlMouvement">
    <h1><?php echo __('Mouvement Alimentation Caisse', array(), 'messages') ?></h1>
    <?php
    $sDatedebut = date("Y-m-d", mktime(0, 0, 0, date("m"), 1, date("Y")));
    $d = new DateTime(date('Y-m-d'));
    $sDateFin = $d->format('Y-m-t');
    ?>
<input type="hidden" value="<?php echo $caisses->getFirst()->getId();?>" id='id_banque'>
    <div id="sf_admin_bar" >
                ...
                <form>
                    <table style="margin-bottom: 0px;" class="table table-bordered table-hover" cellspacing="0">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="2">
                                    <a onclick="" href="<?php echo url_for('mouvementbanciare/journalCaisse') ?>" class="btn btn-xs btn-success">Effacer</a>
                                    <input type="submit" value="Filtrer" class="btn btn-xs btn-success" onclick="goPage(1)">
                                </td>
                            </tr>
                        </tfoot>
                    </table>
...
goPage(1);
    function goPage(page) {
        if ($("#mouvementbanciare_filters_id_banque").val() != '0') {
            $.ajax({
                url: '<?php echo url_for('mouvementbanciare/goPageJournalCaisse') ?>',
                data: 'page=' + page +
                        ...
                        + '&quitance=' + $('#mouvementbanciare_filters_quitance').val(),
