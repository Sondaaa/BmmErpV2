<?php
// Connection Component Binding
Doctrine_Manager::getInstance()->bindComponent('Ristourne', 'doctrine');

/**
 * BaseRistourne
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $id
 * @property string $libelle
 * @property integer $id_sousrubrique
 * @property Sousrubrique $Sousrubrique
 * @property Doctrine_Collection $Tableaubordformation
 * 
 * @method integer             getId()                   Returns the current record's "id" value
 * @method string              getLibelle()              Returns the current record's "libelle" value
 * @method integer             getIdSousrubrique()       Returns the current record's "id_sousrubrique" value
 * @method Sousrubrique        getSousrubrique()         Returns the current record's "Sousrubrique" value
 * @method Doctrine_Collection getTableaubordformation() Returns the current record's "Tableaubordformation" collection
 * @method Ristourne           setId()                   Sets the current record's "id" value
 * @method Ristourne           setLibelle()              Sets the current record's "libelle" value
 * @method Ristourne           setIdSousrubrique()       Sets the current record's "id_sousrubrique" value
 * @method Ristourne           setSousrubrique()         Sets the current record's "Sousrubrique" value
 * @method Ristourne           setTableaubordformation() Sets the current record's "Tableaubordformation" collection
 * 
 * @package    PhpProjectTest
 * @subpackage model
 * @author     Your name here
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class BaseRistourne extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('ristourne');
        $this->hasColumn('id', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'primary' => true,
             'sequence' => 'ristourne_id',
             'length' => 4,
             ));
        $this->hasColumn('libelle', 'string', 255, array(
             'type' => 'string',
             'fixed' => 1,
             'unsigned' => false,
             'notnull' => false,
             'primary' => false,
             'length' => 255,
             ));
        $this->hasColumn('id_sousrubrique', 'integer', 4, array(
             'type' => 'integer',
             'fixed' => 0,
             'unsigned' => false,
             'notnull' => false,
             'primary' => false,
             'length' => 4,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('Sousrubrique', array(
             'local' => 'id_sousrubrique',
             'foreign' => 'id'));

        $this->hasMany('Tableaubordformation', array(
             'local' => 'id',
             'foreign' => 'id_ristourne'));
    }
}